...
// Convert string thành kebab-case để so sánh
...
// Tìm tên thực tế của component trong origin folder
function findRealComponentName(inputName, pattern) {
  const patternDir = path.dirname(pattern);
  const originDir = path.join(ORIGIN_ROOT, patternDir);
  // Check if pattern ends with file extension (có dấu chấm sau {name})
  const patternFileName = path.basename(pattern);
  const isFilePattern = patternFileName.includes('.') && patternFileName !== '{name}';
  
  
  if (!fs.existsSync(originDir)) {
    console.log(`⚠️  Directory doesn't exist: ${originDir}`);
    return inputName; // Fallback to input name if directory doesn't exist
  }
  
  try {
    const entries = fs.readdirSync(originDir);
    // Tìm exact match trước (case sensitive)
    for (const entry of entries) {
      if (entry === inputName) {
        console.log(`✅ Exact match found: "${entry}"`);
        return entry;
      }
    }
    
    // Tìm match bằng cách convert về kebab-case để so sánh
    for (const entry of entries) {
      const entryPath = path.join(originDir, entry);
      const stat = fs.statSync(entryPath);
      
      // Chỉ match đúng type: file cho file patterns, directory cho directory patterns
      const isEntryFile = stat.isFile();
      const isEntryDir = stat.isDirectory();
      
      if ((isFilePattern && isEntryFile) || (!isFilePattern && isEntryDir)) {
        // Bỏ extension để so sánh (nếu có)
        const entryWithoutExt = path.parse(entry).name;
        const entryKebab = toKebabCase(entryWithoutExt);
        if (entryKebab === inputName) {
          console.log(`🎯 Found real name: "${entry}" (from input: "${inputName}"), isFilePattern: ${isFilePattern}`);
          
          // Với file patterns: return name without extension để tránh double extension
          // Với folder patterns: return full name để giữ nguyên case
          if (isFilePattern) {
            return entryWithoutExt; // Return without extension cho file patterns (tránh .hbs.hbs)
          } else {
            return entry; // Return full entry name cho folder patterns (giữ nguyên case)
          }
        }
      }
    }
    ...
    console.log(`⚠️  Error reading directory ${originDir}: ${error.message}`);
  }
  
  console.log(`⚪ No match found, fallback to input: "${inputName}"`);
  return inputName; // Fallback to input name
}

// Enhanced cleanup function để tìm và xóa tất cả variant folders
function cleanupVariantFolders(realName, pattern) {
  const patternDir = path.dirname(pattern);
  const destDir = path.join(DEST_ROOT, patternDir);
  ...
  try {
    ...
    const realNameKebab = toKebabCase(realName);
    ...
    for (const entry of entries) {
      ...
      // Chỉ check folders (directories)
      if (stat.isDirectory()) {
        const entryKebab = toKebabCase(entry);
        
        // Nếu entry match với component (cùng kebab-case) nhưng không phải real name
        if (entryKebab === realNameKebab && entry !== realName) {
          console.log(`🗑️ Removing duplicate variant folder: ${entry} (real name: ${realName})`);
    ...
    console.log(`⚠️ Error during cleanup scan: ${error.message}`);
  }
}
...
function syncComponent(inputName) {
  ...
  for (const pattern of PATTERNS) {
    ...
    if (isVariantPath(pattern)) {
      // Tìm real name trong origin trước
      const realName = findRealComponentName(inputName, pattern);
      
      // Nếu tìm được real name khác với input, chỉ sync với real name
      if (realName !== inputName) {
        console.log(`🎯 Using real name: ${realName} (from input: ${inputName})`);
        const realRel = pattern.replace(/{name}/g, realName);
        const srcReal = path.join(ORIGIN_ROOT, realRel);
        const dstReal = path.join(DEST_ROOT, realRel);
        syncPath(srcReal, dstReal, treatAsDir);
        
        // Enhanced cleanup: tìm tất cả variant folders có sẵn trong destination
        cleanupVariantFolders(realName, pattern);
      } else {
        // Fallback to original logic nếu không tìm được real name
      ...
      // Tìm tên thực tế từ origin trước
      const realName = findRealComponentName(inputName, pattern);
      console.log(`🔍 Pattern: ${pattern}, Real name: ${realName}, Input: ${inputName}`);
      const rel = pattern.replace(/{name}/g, realName);
      const src = path.join(ORIGIN_ROOT, rel);
      // Destination cũng dùng real name để giữ nguyên tên gốc
      const dst = path.join(DEST_ROOT, rel);
      syncPath(src, dst, treatAsDir);
    }
  }
