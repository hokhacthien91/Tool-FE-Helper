(()=>{var On=Object.defineProperty,Vn=Object.defineProperties;var Gn=Object.getOwnPropertyDescriptors;var Xo=Object.getOwnPropertySymbols;var _n=Object.prototype.hasOwnProperty,Jn=Object.prototype.propertyIsEnumerable;var Zo=(h,w,f)=>w in h?On(h,w,{enumerable:!0,configurable:!0,writable:!0,value:f}):h[w]=f,Je=(h,w)=>{for(var f in w||(w={}))_n.call(w,f)&&Zo(h,f,w[f]);if(Xo)for(var f of Xo(w))Jn.call(w,f)&&Zo(h,f,w[f]);return h},Xe=(h,w)=>Vn(h,Gn(w));function Ko(){function h(w){try{let f=w.target&&w.target.closest?w.target.closest("button"):null;if(!f)return;let v=f.closest?f.closest(".issue"):null;if(!v||!(f.closest&&f.closest(".issue-actions")))return;let C=f.getAttribute("data-id")||v.getAttribute("data-issue-id");if(!C)return;parent.postMessage({pluginMessage:{type:"select-node",id:C}},"*")}catch(f){console.error("autoSelectNodeFromIssueClick error:",f)}}document.addEventListener("click",h,!0)}function ye(h,w,f){if(!w)return;let v=document.querySelector(`.issue[data-issue-id="${h}"]`);if(!v){let V=document.querySelector(`button.btn-fix[data-id="${h}"]`);V&&(v=V.closest(".issue"))}if(!v)return;let I=v.querySelector(".fix-message");I&&I.remove();let C=document.createElement("div");C.className="fix-message",C.style.cssText=`
    ...
    `,re.appendChild(K),document.body.appendChild(re);let ie=K.querySelector("#fix-font-family"),oe=K.querySelector("#fix-font-size"),de=K.querySelector("#fix-font-weight"),te=K.querySelector("#fix-line-height"),le=K.querySelector("#fix-letter-spacing"),A=K.querySelector("#choose-typo-style-btn"),pe=K.querySelector("#typography-fix-modal-cancel-btn"),_=K.querySelector("#typography-fix-modal-apply-btn"),ue=K.querySelector(".modal-close");setTimeout(()=>{ie.focus()},100);let S=()=>{re.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{re.parentNode&&re.remove()},200)};pe.onclick=S,ue.onclick=S,re.onclick=J=>{J.target===re&&S()},A.onclick=()=>{Yn(w,J=>{J&&(ie.value=J.fontFamily||"",oe.value=J.fontSize||"",de.value=J.fontWeight||"Regular",te.value=J.lineHeight||"",le.value=J.letterSpacing||"0")})},_.onclick=()=>{let J={fontFamily:ie.value.trim(),fontSize:oe.value.trim(),fontWeight:de.value,lineHeight:te.value.trim(),letterSpacing:le.value.trim()};if(!J.fontFamily){ie.focus(),ie.style.borderColor="#ff3b30",setTimeout(()=>{ie.style.borderColor="#0071e3"},2e3);return}if(!J.fontSize||isNaN(parseFloat(J.fontSize))){oe.focus(),oe.style.borderColor="#ff3b30",setTimeout(()=>{oe.style.borderColor="#0071e3"},2e3);return}S(),ye(h.id,"\u23F3 Fixing...",!0),parent.postMessage({pluginMessage:{type:"fix-issue",issue:h,fixData:J}},"*")}}function Xn(h,w){if(h===w)return 100;let f=100,v=Math.abs(h-w);return Math.max(0,Math.round((1-v/f)*100))}function tn(h,w,f,v){let I=document.createElement("div");I.className="modal-overlay",I.id="spacing-picker-modal-overlay";let C=document.createElement("div");C.className="modal-dialog",C.style.maxWidth="400px";let q=v.map(T=>`
    ...
    `,I.appendChild(C),document.body.appendChild(I);let D=C.querySelector("#spacing-picker-modal-cancel-btn"),B=C.querySelector(".modal-close"),P=C.querySelectorAll(".spacing-picker-item"),U=()=>{I.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{I.parentNode&&I.remove()},200)};D.onclick=U,B.onclick=U,I.onclick=T=>{T.target===I&&U()},P.forEach(T=>{T.onclick=O=>{O.preventDefault(),O.stopPropagation();let re=parseInt(T.getAttribute("data-value"),10);U(),Nt(h,w,f,re)}})}function Nt(h,w,f,v,I=[],C={}){let{onApply:q,onIgnore:V,onCancel:D,showIgnore:B=!1,progress:P}=C,U=String(w||"").replace(/([A-Z])/g," $1").replace(/^./,Y=>Y.toUpperCase()).trim(),O=(I.length>0?I:[0,4,8,12,16,24,32,40,48,64,72,80,88,96]).map(Y=>({value:Y,similarity:Xn(f,Y),diff:Math.abs(f-Y)})).sort((Y,me)=>v!==void 0&&Y.value===v?-1:v!==void 0&&me.value===v?1:Y.diff-me.diff).slice(0,5);if(O.length===0){alert("No spacing values available");return}let re=O[0].value,K=(Y,me)=>{let xe=Y.value!==f;return`
    ...
    `},ie=document.createElement("div");ie.className="modal-overlay",ie.id="spacing-fix-confirm-modal-overlay";let oe=document.createElement("div");oe.className="modal-dialog",oe.style.maxWidth="420px";let de=P?`<div style="margin-bottom: 12px; padding: 8px 12px; background: #e3f2fd; border-radius: 6px; font-size: 13px; color: #1976d2; font-weight: 600;">Progress: ${P.current}/${P.total}</div>`:"",te=O.map((Y,me)=>K(Y,me===0)).join("");oe.innerHTML=`
      <div class="modal-header">
        <button class="modal-close" aria-label="Close">\xD7</button>
    ...
    `,ie.appendChild(oe),document.body.appendChild(ie);let le=oe.querySelector("#spacing-fix-confirm-cancel-btn"),A=oe.querySelector("#spacing-fix-confirm-apply-btn"),pe=oe.querySelector("#spacing-fix-ignore-btn"),_=oe.querySelector(".modal-close"),ue=oe.querySelector("#spacing-options-container"),S=Y=>{re=Y,ue.querySelectorAll(".spacing-option-item").forEach(xe=>{let Q=parseInt(xe.getAttribute("data-value"),10)===Y;xe.style.border=Q?"2px solid #0071e3":"2px solid #e0e0e0",xe.style.background=Q?"#e3f2fd":"white";let Ze=xe.querySelector('input[type="radio"]');Ze&&(Ze.checked=Q)})};(()=>{ue.querySelectorAll(".spacing-option-item").forEach(me=>{me.onclick=xe=>{xe.preventDefault();let je=parseInt(me.getAttribute("data-value"),10);S(je)}})})();let se=()=>{ie.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{ie.parentNode&&ie.remove()},200)};le.onclick=()=>{se(),D&&typeof D=="function"&&D()},_.onclick=()=>{se(),D&&typeof D=="function"&&D()},ie.onclick=Y=>{Y.target===ie&&(se(),D&&typeof D=="function"&&D())},A.onclick=()=>{se(),ye(h.id,"\u23F3 Fixing spacing...",!0),parent.postMessage({pluginMessage:{type:"fix-spacing-issue",issue:h,propertyName:w,value:re}},"*"),q&&typeof q=="function"&&q()},pe&&(pe.onclick=()=>{se(),V&&typeof V=="function"&&V()})}function ht(h){try{let w=parseInt(h.slice(1,3),16),f=parseInt(h.slice(3,5),16),v=parseInt(h.slice(5,7),16);return(w*299+f*587+v*114)/1e3>128?"#000000":"#ffffff"}catch(w){return"#000000"}}function Zn(h,w){let f=h.replace("#",""),v=w.replace("#",""),I=parseInt(f.substr(0,2),16),C=parseInt(f.substr(2,2),16),q=parseInt(f.substr(4,2),16),V=parseInt(v.substr(0,2),16),D=parseInt(v.substr(2,2),16),B=parseInt(v.substr(4,2),16);return Math.sqrt(Math.pow(I-V,2)+Math.pow(C-D,2)+Math.pow(q-B,2))}function Kn(h,w){let v=Zn(h,w);return Math.round((1-v/441.67)*100)}function Jt(h,w,f,v="",I=""){return ht(h),`
    ...
    `,I.appendChild(C),document.body.appendChild(I);let V=C.querySelector("#color-picker-modal-cancel-btn"),D=C.querySelector(".modal-close"),B=C.querySelectorAll(".color-picker-item"),P=()=>{I.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{I.parentNode&&I.remove()},200)};V.onclick=P,D.onclick=P,I.onclick=U=>{U.target===I&&P()},B.forEach(U=>{U.onclick=T=>{T.preventDefault(),T.stopPropagation();let O=U.getAttribute("data-color");P(),Bt(h,w,O,v)}})}function Bt(h,w,f,v={},I=[],C={}){let{onApply:q,onIgnore:V,onCancel:D,showIgnore:B=!1,progress:P}=C,U=I.length>0?I:Object.keys(v);U.length===0&&f&&(U=[f]);let T=U.map(se=>({color:se,name:v[se]||se,similarity:Kn(w,se)})).sort((se,Y)=>f&&se.color===f?-1:f&&Y.color===f?1:Y.similarity-se.similarity).slice(0,5);if(T.length===0){alert("No colors available");return}let O=T[0].color,re=(se,Y)=>`
    ...
    `,K=document.createElement("div");K.className="modal-overlay",K.id="color-fix-confirm-modal-overlay";let ie=document.createElement("div");ie.className="modal-dialog",ie.style.maxWidth="420px";let oe=P?`<div style="margin-bottom: 12px; padding: 8px 12px; background: #e3f2fd; border-radius: 6px; font-size: 13px; color: #1976d2; font-weight: 600;">Progress: ${P.current}/${P.total}</div>`:"",de=T.map((se,Y)=>re(se,Y===0)).join("");ie.innerHTML=`
      <div class="modal-header">
        <button class="modal-close" aria-label="Close">\xD7</button>
    ...
    `,K.appendChild(ie),document.body.appendChild(K);let te=ie.querySelector("#color-fix-confirm-cancel-btn"),le=ie.querySelector("#color-fix-confirm-apply-btn"),A=ie.querySelector("#color-fix-ignore-btn"),pe=ie.querySelector(".modal-close"),_=ie.querySelector("#color-options-container"),ue=se=>{O=se,_.querySelectorAll(".color-option-item").forEach(me=>{let je=me.getAttribute("data-color")===se;me.style.border=je?"2px solid #0071e3":"2px solid #e0e0e0",me.style.background=je?"#e3f2fd":"white";let Q=me.querySelector('input[type="radio"]');Q&&(Q.checked=je)})};(()=>{_.querySelectorAll(".color-option-item").forEach(Y=>{Y.onclick=me=>{me.preventDefault();let xe=Y.getAttribute("data-color");ue(xe)}})})();let J=()=>{K.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{K.parentNode&&K.remove()},200)};te.onclick=()=>{J(),D&&typeof D=="function"&&D()},pe.onclick=()=>{J(),D&&typeof D=="function"&&D()},K.onclick=se=>{se.target===K&&(J(),D&&typeof D=="function"&&D())},le.onclick=()=>{J(),ye(h.id,"\u23F3 Fixing color...",!0),parent.postMessage({pluginMessage:{type:"fix-color-issue",issue:h,color:O}},"*"),q&&typeof q=="function"&&q()},A&&(A.onclick=()=>{J(),V&&typeof V=="function"&&V()})}function Yt({reportData:h,getTypeDisplayName:w}){let f=h||{},v=f.issues||[],I=f.tokens||null,C=f.timestamp||Date.now(),q=new Date(C).toLocaleString("vi-VN"),V="Design Review",D=P=>{try{return typeof w=="function"?w(P):String(P||"")}catch(U){return String(P||"")}},B=`<!DOCTYPE html>
<html lang="vi">
<head>
  <meta charset="UTF-8">
...
<body>
  <div class="container">
    ...
    </div>`;if(v&&v.length>0){let P={error:v.filter(T=>T.severity==="error").length,warn:v.filter(T=>T.severity==="warn").length,total:v.length},U=v.reduce((T,O)=>(T[O.type]=T[O.type]||[],T[O.type].push(O),T),{});B+=`
    <div class="section">
    ...
    <div class="section">
      ...
      </div>`;for(let[T,O]of Object.entries(U)){let re=u(D(T));B+=`
      <div class="issue-group collapsed" data-type="${T}" data-label="${re}">
        <div class="issue-group-header">
          <button class="issue-group-toggle" type="button">+</button>
          <span>${re} (${O.length})</span>
        </div>
    ...
    <div class="section">
      ...
      <div class="issue-group collapsed">
        ...
        <div class="issue-group-content">`,!T.values||T.values.length===0){B+=`
          ...
          <div class="token-list">`,T.values.forEach(O=>{let re=O.value,K=typeof O.totalNodes=="number"?O.totalNodes:O.nodes?O.nodes.length:0,ie=O.colorType||"";if(U==="colors")B+=`
          <div class="token-item">
            <div class="token-value">
              <span class="token-color-preview" style="background-color: ${u(re)}"></span>
              ${u(re)}
          ...
          <div class="token-item">
            <div class="token-value">
              <span class="token-color-preview" style="background: ${u(re)}"></span>
              ${u(re)}
              ${ie?`<span class="token-color-type">${u(ie)}</span>`:""}
            </div>
            ${K>1?`<div class="token-node-count">Used in ${K} nodes</div>`:""}
          ...
          <div class="token-item">
            <div class="token-value">${u(String(re))}</div>
            ${K>1?`<div class="token-node-count">Used in ${K} nodes</div>`:""}
            ${oe}
          </div>`}}),B+=`
          </div>
  ...
  <\/script>
...
</html>`,B}function Qn(){let h=new Date,w=h.getFullYear(),f=String(h.getMonth()+1).padStart(2,"0"),v=String(h.getDate()).padStart(2,"0"),I=String(h.getHours()).padStart(2,"0"),C=String(h.getMinutes()).padStart(2,"0"),q=String(h.getSeconds()).padStart(2,"0");return`design-review-report-${w}-${f}-${v}-${I}-${C}-${q}`}function el(h,w){if(!h||!Array.isArray(h.colors))return h;let f=w||{};return Xe(Je({},h),{colors:h.colors.map(v=>Xe(Je({},v),{name:f[String(v.value).toUpperCase()]||"No name"}))})}function nn({format:h,reportData:w,getTypeDisplayName:f,filenameBase:v,colorNameMap:I}={}){var V,D;let C=w||{};if(!C.issues&&!C.tokens){alert("No data to export!");return}let q=v||Qn();if(h==="html"){let B=Yt({reportData:C,getTypeDisplayName:f}),P=new Blob([B],{type:"text/html"}),U=URL.createObjectURL(P),T=document.createElement("a");T.href=U,T.download=`${q}.html`,T.click(),URL.revokeObjectURL(U);return}if(h==="pdf"){let B=Yt({reportData:C,getTypeDisplayName:f});try{let P=window.open("","_blank");if(!P){alert("Popup blocked. Downloading HTML - you can open the file and select Print to create PDF.");let U=new Blob([B],{type:"text/html"}),T=URL.createObjectURL(U),O=document.createElement("a");O.href=T,O.download=`${q}.html`,O.click(),URL.revokeObjectURL(T);return}P.document.open(),P.document.write(B),P.document.close(),P.onload=()=>{setTimeout(()=>{P.print()},250)},setTimeout(()=>{P.document&&P.document.readyState==="complete"&&P.print()},500)}catch(P){console.error("Error opening print window:",P),alert("Cannot open print window. Downloading HTML - you can open the file and select Print to create PDF.");let U=new Blob([B],{type:"text/html"}),T=URL.createObjectURL(U),O=document.createElement("a");O.href=T,O.download=`${q}.html`,O.click(),URL.revokeObjectURL(T)}return}if(h==="json"){console.log("Export JSON - colorNameMap:",I),console.log("Export JSON - tokens.colors:",(V=C.tokens)==null?void 0:V.colors);let B=Xe(Je({},C),{tokens:el(C.tokens,I)});console.log("Export JSON - enriched colors:",(D=B.tokens)==null?void 0:D.colors);let P=JSON.stringify(B,null,2),U=new Blob([P],{type:"application/json"}),T=URL.createObjectURL(U),O=document.createElement("a");O.href=T,O.download=`${q}.json`,O.click(),URL.revokeObjectURL(T);return}}function ln({maxHistory:h=10,postPluginMessage:w,getCurrentReportData:f,setIsViewingTokens:v,renderResults:I,renderTokens:C}={}){let q=[],V=!1,D=te=>{try{typeof w=="function"&&w(te)}catch(le){console.error("scanHistory postPluginMessage error:",le)}};function B(te){q=(Array.isArray(te)?te:[]).slice(0,h)}function P(){q=[],V=!1}function U(){return q||[]}function T(te,le,A,pe){try{if(le==="issues"&&(!A.issues||A.issues.length===0)){console.log("Skipping save - no issues data");return}if(le==="tokens"&&(!A.tokens||Object.keys(A.tokens).length===0)){console.log("Skipping save - no tokens data");return}let _={id:Date.now().toString(),mode:te,type:le,timestamp:new Date().toISOString(),context:pe||null,data:{issues:A.issues||null,tokens:A.tokens||null,issuesCount:A.issues?A.issues.length:0,tokensCount:A.tokens?Object.keys(A.tokens).reduce((S,J)=>{var se;return S+(((se=A.tokens[J])==null?void 0:se.length)||0)},0):0}};q.unshift(_),q=q.slice(0,h);let ue=document.getElementById("history-panel");ue&&ue.style.display!=="none"&&oe(),D({type:"save-history-entry",entry:_})}catch(_){console.error("Error saving scan history:",_)}}function O(){D({type:"get-history"})}function re(){try{let te=U();if(te.length>0){let le=te[0],A=document.querySelector(`input[name="scope"][value="${le.mode}"]`);A&&(A.checked=!0,console.log("Loaded last scan mode:",le.mode))}}catch(te){console.error("Error loading last scan mode:",te)}}function K(){V||(re(),V=!0)}function ie(te){try{let le=new Date(te),pe=new Date-le,_=Math.floor(pe/6e4),ue=Math.floor(pe/36e5),S=Math.floor(pe/864e5);return _<1?"Just now":_<60?`${_} minutes ago`:ue<24?`${ue} hours ago`:S<7?`${S} days ago`:le.toLocaleString("en-US",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"})}catch(le){return te}}function oe(){let te=document.getElementById("history-list");if(!te){console.error("history-list element not found");return}let le=U();if(console.log("Rendering scan history:",le.length,"entries"),le.length===0){te.innerHTML=`
              <div class="history-empty">
                <div class="icon">\u{1F4CB}</div>
                <p>No scan history</p>
                <p style="font-size: 11px; margin-top: 8px;">Scans will be saved automatically</p>
              </div>
            `;return}te.innerHTML=le.map(A=>{var Y,me;let pe=ie(A.timestamp),_=A.mode==="page"?"Page":"Selection",ue=A.type==="issues"?"Issues":"Tokens",S=A.type==="issues"?"issues":"tokens",J=A.context&&A.context.label?A.context.label:`${_} scan`,se="";if(A.type==="issues"){let xe=((Y=A.data.issues)==null?void 0:Y.filter(Q=>Q.severity==="error").length)||0,je=((me=A.data.issues)==null?void 0:me.filter(Q=>Q.severity==="warn").length)||0;se=`
                <span>\u274C ${xe} errors</span>
        ...
        " onmouseover="this.style.borderColor='#0071e3'; this.style.boxShadow='0 2px 8px rgba(0,113,227,0.2)'" onmouseout="this.style.borderColor='${ee?"#0071e3":"#ddd"}'; this.style.boxShadow='none'">
          ...
          <div style="font-size: 11px; color: #666; padding-top: 8px; border-top: 1px solid #eee;">
            ...
            <div style="padding-left: 8px; line-height: 1.8;">
              ...
              \u2022 Letter Spacing: <code style="${ce?z:Z}">${ce?"\u26A0 ":"\u2713 "}${u(N.letterSpacing||"0")}</code>
            </div>
          </div>
        </div>
      `},H=x.map(N=>F(N)).join("");l.innerHTML=`
      <div class="modal-header">
        <button class="modal-close" aria-label="Close">\xD7</button>
    ...
    `,d.appendChild(y),document.body.appendChild(d);let F=y.querySelector("#text-size-fix-confirm-cancel-btn"),H=y.querySelector("#text-size-fix-confirm-apply-btn"),k=y.querySelector("#text-size-fix-ignore-btn"),$=y.querySelector(".modal-close"),j=y.querySelector("#text-size-options-container"),L=g,W=E=>{a=E,j.querySelectorAll(".text-size-option-item").forEach(X=>{let c=parseInt(X.getAttribute("data-size"),10)===E;X.style.border=c?"2px solid #0071e3":"2px solid #e0e0e0",X.style.background=c?"#e3f2fd":"white";let ce=X.querySelector('input[type="radio"]');ce&&(ce.checked=c)})};(()=>{j.querySelectorAll(".text-size-option-item").forEach(M=>{M.onclick=X=>{X.preventDefault();let ne=parseInt(M.getAttribute("data-size"),10);W(ne)}})})();let N=()=>{d.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{d.parentNode&&d.remove(),l&&(l.style.display="block")},200)},ee=()=>{d.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{d.parentNode&&d.remove(),l&&l.parentNode&&(l.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{l.parentNode&&l.remove()},200))},200)};F.onclick=()=>{L&&L.onCancel?(ee(),window.pendingTextSizeFixAllCallbacks=null,L.onCancel()):N()},$.onclick=()=>{L&&L.onCancel?(ee(),window.pendingTextSizeFixAllCallbacks=null,L.onCancel()):N()},d.onclick=E=>{E.target===d&&(L&&L.onCancel?(ee(),window.pendingTextSizeFixAllCallbacks=null,L.onCancel()):N())},k&&(k.onclick=()=>{ee(),L&&L.onIgnore&&(window.pendingTextSizeFixAllCallbacks=null,L.onIgnore())}),H.onclick=()=>{ee(),ye(e.id,"\u23F3 Fixing text size...",!0),n&&n.id?parent.postMessage({pluginMessage:{type:"apply-figma-text-style",issue:e,styleId:n.id,styleName:n.name}},"*"):parent.postMessage({pluginMessage:{type:"fix-text-size-issue",issue:e,fontSize:a}},"*"),L&&L.onApply&&(window.pendingTextSizeFixAllCallbacks=null,L.onApply())}}function uo(e){parent.postMessage({pluginMessage:{type:"get-contrast-colors",issue:e}},"*"),window.pendingContrastIssue=e}function qt(e){let t=ct(e);if(!t){alert("No suitable color found that passes contrast requirements");return}go(e,e.textColor,t,null)}function go(e,t,o,l){let n=e.backgroundColor||"#FFFFFF",s=e.minContrast||4.5,r=(E,M)=>{let X=E.replace("#",""),ne=M.replace("#",""),c=parseInt(X.substr(0,2),16),ce=parseInt(X.substr(2,2),16),Z=parseInt(X.substr(4,2),16),z=parseInt(ne.substr(0,2),16),G=parseInt(ne.substr(2,2),16),fe=parseInt(ne.substr(4,2),16);return Math.sqrt(Math.pow(c-z,2)+Math.pow(ce-G,2)+Math.pow(Z-fe,2))},i=(E,M)=>{let ne=r(E,M);return Math.round((1-ne/441.67)*100)},p=Object.keys(_).map(E=>{let M=Tt(E,n);return{color:E,name:_[E]||E,contrast:M,passes:M>=s,similarity:i(t,E),distance:r(t,E)}}).filter(E=>E.passes).sort((E,M)=>o&&E.color===o?-1:o&&M.color===o?1:E.distance-M.distance).slice(0,5),d=p.length>0?p[0].color:o,y=(E,M)=>`
...
The ${l.length} issues will need to be fixed manually.`)||parent.postMessage({pluginMessage:{type:"fix-all-issues",issues:o,issueType:e}},"*")}function An(e){console.log("filterAndSearchIssues called",{totalIssues:e.length,currentFilter:Ae,currentSearch:Ce});let t=e;if(Ae!=="all"&&(t=t.filter(o=>o.severity===Ae),console.log("After severity filter:",t.length)),Ce.trim()){let o=Ce.toLowerCase();t=t.filter(l=>{let n=(l.message||"").toLowerCase(),s=(l.nodeName||"").toLowerCase(),r=(l.type||"").toLowerCase();return n.includes(o)||s.includes(o)||r.includes(o)}),console.log("After search filter:",t.length)}return console.log("Final filtered issues:",t.length),t}function Ke(e=[],t=!1,o={}){let{skipSave:l=!1,restoreTimestamp:n=null}=o;We("issues"),document.getElementById("issues-count").textContent=e.length,e&&Array.isArray(e)&&e.forEach(k=>{ue[k.id]===!0&&(k.ignored=!0,k.originalSeverity||(k.originalSeverity=k.severity),k.severity="info")});let s=S.issues!==e;S.issues=e;let r=n||new Date().toISOString();S.timestamp=r,bt=!1,(t||s)&&(console.log("Resetting filters for new data"),Ae="all",Oe="all",De&&(De.value=""),Ce="",Be&&(Be.style.display="none"),we&&we.length>0&&we.forEach(k=>{k.classList.remove("active"),k.getAttribute("data-filter")==="all"&&k.classList.add("active")}));let i=document.getElementById("filter-controls"),a=document.getElementById("color-type-filter"),p=document.getElementById("filter-buttons");i.style.display=e.length>0?"flex":"none",a.style.display="none",p.style.display="flex";let d=document.getElementById("export-group");d.style.display=e.length>0?"flex":"none",console.log("About to filter with:",{currentFilter:Ae,currentSearch:Ce});let y=An(e),g=new Set;if(s||(document.querySelectorAll(".issue-group").forEach($=>{let j=$.getAttribute("data-issue-type");j&&!$.classList.contains("collapsed")&&g.add(j)}),console.log("Saved expanded groups:",Array.from(g))),kt("issues"),y.length===0&&e.length===0){oe.innerHTML=`
              <div class="empty-state success">
                <div class="icon">\u2705</div>
                <p><strong>No issues found!</strong></p>
                <p style="margin-top: 8px; font-size: 12px;">Your design passed all configured checks.</p>
              </div>
            `;return}if(y.length===0&&e.length>0&&Ae!=="all"){oe.innerHTML=`
              <div class="empty-state">
                <div class="icon">\u{1F50D}</div>
                <p><strong>No results found</strong></p>
                <p style="margin-top: 8px; font-size: 12px;">Try changing the filter or search keyword.</p>
              </div>
            `;return}let m={error:y.filter(k=>k.severity==="error"&&!k.ignored).length,warn:y.filter(k=>k.severity==="warn"&&!k.ignored).length,total:y.length,originalTotal:e.length},x=document.createElement("div");x.className="results-header",x.innerHTML=`
            <h3>Check Result</h3>
          ...
          `,oe.appendChild(x);let b=y.reduce((k,$)=>(k[$.type]=k[$.type]||[],k[$.type].push($),k),{}),F=e.reduce((k,$)=>(k[$.type]=k[$.type]||[],k[$.type].push($),k),{}),H=["naming","autolayout","spacing","color","typography","typography-style","typography-check","line-height","position","duplicate","group","component","empty-frame","nested-group","contrast","text-size-mobile"];for(let k of H){let $=b[k]||[],j=$.length,L=$.filter(c=>c.ignored?!1:c.severity==="error"||c.severity==="warn").length;if(j===0&&e.length===0||j===0&&Ae!=="all")continue;let W=document.createElement("div"),ge=g.has(k);W.className=ge?"issue-group":"issue-group collapsed",W.setAttribute("data-issue-type",k);let N=document.createElement("div");N.className="issue-group-header";let ee=(F[k]||[]).some(c=>{try{if(!c)return!1;switch(c.type){case"color":return ft(c)!==null;case"spacing":return yt(c)!==null;case"autolayout":return typeof it=="function"&&it(c)!==null;case"text-size-mobile":return typeof ot=="function"&&ot(c)!==null;case"contrast":return typeof ct=="function"&&ct(c)!==null;case"typography-style":case"typography-check":let ce=c.bestMatch&&c.bestMatch!==null&&c.bestMatch!==void 0&&c.bestMatch.name&&typeof c.bestMatch.name=="string"&&c.bestMatch.name.trim().length>0&&St(c);return k==="typography-style"&&console.log("[hasSuggestFixButton] Typography-style issue",c.id,"hasBestMatch:",ce,"bestMatch:",c.bestMatch),ce;case"position":return typeof tt=="function"&&tt(c)!==null;case"duplicate":case"component":return typeof at=="function"&&at(c)!==null;case"group":return!0;case"empty-frame":return typeof rt=="function"&&rt(c)!==null;default:return!1}}catch(ce){return console.error("[hasSuggestFixButton] Error checking issue:",c,ce),!1}});k==="typography-style"&&console.log("[hasSuggestFixButton] Type:",k,"hasSuggestFixButton:",ee,"allGrouped[type]:",F[k]),N.innerHTML=`
            ...
            `;let E=N.querySelector(".issue-group-toggle"),M=()=>{W.classList.contains("collapsed")?(W.classList.remove("collapsed"),ne.style.display="block",setTimeout(()=>{ne.style.opacity="1"},10)):(ne.style.opacity="0",setTimeout(()=>{W.classList.add("collapsed"),ne.style.display="none"},200))};E.onclick=c=>{c.stopPropagation(),M()},N.onclick=c=>{c.target!==E&&!E.contains(c.target)&&M()};let X=N.querySelector(".btn-fix-all");console.log("[Fix All] Setting up button for type:",k,"btnFixAll found:",!!X),X?X.onclick=c=>{try{console.log("[Fix All] ========== BUTTON CLICKED =========="),console.log("[Fix All] Button clicked for type:",k),console.log("[Fix All] Event:",c),c.preventDefault(),c.stopPropagation(),console.log("[Fix All] All issues in group:",F[k]),console.log("[Fix All] allGrouped[type] length:",(F[k]||[]).length);let ce=(F[k]||[]).filter(Z=>{if(!Z)return console.log("[Fix All] Filter: issue is null/undefined"),!1;switch(console.log("[Fix All] Filter: checking issue",Z.id,"type:",Z.type,"bestMatch:",Z.bestMatch),Z.type){case"color":return ft(Z)!==null;case"spacing":return yt(Z)!==null;case"autolayout":return typeof it=="function"&&it(Z)!==null;case"text-size-mobile":return typeof ot=="function"&&ot(Z)!==null;case"contrast":return typeof ct=="function"&&ct(Z)!==null;case"typography-style":case"typography-check":let z=Z.bestMatch&&Z.bestMatch!==null&&Z.bestMatch!==void 0&&Z.bestMatch.name&&typeof Z.bestMatch.name=="string"&&Z.bestMatch.name.trim().length>0&&St(Z);return console.log("[Fix All] Filter: typography issue",Z.id,"hasValidBestMatch:",z,"bestMatch:",Z.bestMatch),z;case"position":return typeof tt=="function"&&tt(Z)!==null;case"duplicate":case"component":return typeof at=="function"&&at(Z)!==null;case"group":return!0;case"empty-frame":return typeof rt=="function"&&rt(Z)!==null;default:return!1}});if(console.log("[Fix All] Filtered issues with suggest fix:",ce),console.log("[Fix All] Issues count:",ce.length),console.log("[Fix All] Filtered issues count:",ce.length),ce.length===0){console.log("[Fix All] No issues with suggest fix available"),alert("No issues with suggest fix available");return}console.log("[Fix All] Starting handleFixAllWithSuggestFix with",ce.length,"issues"),Tn(k,ce)}catch(ce){console.error("[Fix All] ERROR in button onclick:",ce),console.error("[Fix All] Error stack:",ce.stack),alert("Error: "+ce.message)}}:console.log("[Fix All] Button not found for type:",k),W.appendChild(N);let ne=document.createElement("div");if(ne.className="issue-group-content",ge?(ne.style.display="block",ne.style.opacity="1"):ne.style.display="none",j===0){let c=document.createElement("div");c.className="issue info",c.style.opacity="0.7",c.innerHTML=`
              ...
              `,ne.appendChild(c)}else b[k].forEach((c,ce)=>{let Z=ce+1,z=ue[c.id]===!0;if(z&&(c.originalSeverity||(c.originalSeverity=c.severity),c.severity="info",c.ignored=!0),c.type==="typography-check"){let R=document.createElement("div");ne.appendChild(R);let ae=cn(c);if(ae){let he=document.createElement("span");he.className="issue-number",he.textContent=`#${Z}`,he.style.cssText="position: absolute; left: 8px; top: 8px; font-weight: bold; opacity: 0.5; font-size: 11px;",ae.style.position="relative",ae.style.paddingLeft="40px",ae.insertBefore(he,ae.firstChild),ne.replaceChild(ae,R)}return}let G=document.createElement("div"),fe=z?"info":c.severity;G.className=`issue ${fe}`,G.setAttribute("data-issue-id",c.id);let ke=u(c.message);if(c.type==="contrast"){let R=[];if(c.textColor&&R.push(`Text color: <code style="background: ${u(c.textColor)}; padding: 2px 6px; border-radius: 3px; color: ${ht(c.textColor)};">${u(c.textColor)}</code> (${c.textColorNode||c.nodeName||"Unnamed"})`),c.backgroundColor){let ae="Background:",he="",Ne="";c.isGradient&&c.gradientString?(ae="Background (gradient):",he=`<code style="background: ${u(c.backgroundColor)}; padding: 2px 6px; border-radius: 3px; color: ${ht(c.backgroundColor)}; font-family: 'SF Mono', Monaco, monospace; font-size: 11px;">${u(c.gradientString)}</code>`,Ne=" <span style='font-size: 11px; color: #999;'>(average: "+u(c.backgroundColor)+")</span>"):he=`<code style="background: ${u(c.backgroundColor)}; padding: 2px 6px; border-radius: 3px; color: ${ht(c.backgroundColor)};">${u(c.backgroundColor)}</code>`,c.fromSibling&&(Ne+=" <span style='font-size: 11px; color: #3b82f6;'>(from sibling layer)</span>"),R.push(`${ae} ${he}${Ne} (${c.backgroundColorNode||"Unknown"})`)}R.length>0&&(ke+=`<div style="margin-top: 8px; font-size: 12px; color: #666;">${R.join(" | ")}</div>`)}G.innerHTML=`
                <div class="issue-header">
                  <div>
          ...
          `,de.appendChild(H);for(let[$,j]of Object.entries(F)){let L=document.createElement("div");L.className="issue-group collapsed";let W=document.createElement("div");W.className="issue-group-header",W.innerHTML=`
            ...
            `;let ge=document.createElement("div");ge.className="issue-group-content",ge.style.display="none";let N=W.querySelector(".issue-group-toggle"),ee=()=>{L.classList.contains("collapsed")?(L.classList.remove("collapsed"),ge.style.display="block",setTimeout(()=>{ge.style.opacity="1"},10)):(ge.style.opacity="0",setTimeout(()=>{L.classList.add("collapsed"),ge.style.display="none"},200))};if(N.onclick=M=>{M.stopPropagation(),ee()},W.onclick=M=>{M.target!==N&&!N.contains(M.target)&&ee()},L.appendChild(W),Array.isArray(j.values)&&j.values.length>0){let M=document.createElement("div");M.className="token-list",j.values.forEach((X,ne)=>{let c=ne+1,ce=document.createElement("div");ce.className="token-item";let Z=X.value,z=X.nodes||[],G=z.length>0?z[0]:null,fe=typeof X.totalNodes=="number"?X.totalNodes:z.length,ke=X.colorType||null,ze="";if($==="colors"){let Ye=ke?`<span class="token-color-type">${u(ke)}</span>`:"";ze=`
                ...
                `}else ze=`<span class="token-number">#${c}</span><code>${u(String(Z))}</code>`;let Te=X._matchedBy||[],Ee=X._matchedNodeNames||[],nt=Te.includes("nodeName"),Re="";G&&G.name&&(nt&&Ee.length>0?Re=`<div class="token-node-name token-matched-by-name">
                  ...
                  </div>`:Re=`<div class="token-node-name">Node: ${u(G.name)}</div>`);let Ue="";if($==="fontWeight"){let R=Array.isArray(X.fontFamilies)?X.fontFamilies:null;if(!R){let ae={};z.forEach(he=>{let Ne=he&&he.fontFamily?String(he.fontFamily):"Unknown";ae[Ne]=(ae[Ne]||0)+1}),R=Object.entries(ae).map(([he,Ne])=>({family:he,count:Ne})).sort((he,Ne)=>Ne.count-he.count||he.family.localeCompare(Ne.family))}Array.isArray(R)&&R.length>0&&(Ue=`
