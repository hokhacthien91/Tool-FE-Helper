(()=>{var ol=Object.defineProperty,nl=Object.defineProperties;var ll=Object.getOwnPropertyDescriptors;var sn=Object.getOwnPropertySymbols;var sl=Object.prototype.hasOwnProperty,il=Object.prototype.propertyIsEnumerable;var an=(S,C,h)=>C in S?ol(S,C,{enumerable:!0,configurable:!0,writable:!0,value:h}):S[C]=h,Ke=(S,C)=>{for(var h in C||(C={}))sl.call(C,h)&&an(S,h,C[h]);if(sn)for(var h of sn(C))il.call(C,h)&&an(S,h,C[h]);return S},ot=(S,C)=>nl(S,ll(C));function rn(){function S(C){try{let h=C.target&&C.target.closest?C.target.closest("button"):null;if(!h)return;let w=h.closest?h.closest(".issue"):null;if(!w||!(h.closest&&h.closest(".issue-actions")))return;let I=h.getAttribute("data-id")||w.getAttribute("data-issue-id");if(!I)return;parent.postMessage({pluginMessage:{type:"select-node",id:I}},"*")}catch(h){console.error("autoSelectNodeFromIssueClick error:",h)}}document.addEventListener("click",S,!0)}function re(S,C,h){if(!C)return;let w=document.querySelector(`.issue[data-issue-id="${S}"]`);if(!w){let _=document.querySelector(`button.btn-fix[data-id="${S}"]`);_&&(w=_.closest(".issue"))}if(!w)return;let $=w.querySelector(".fix-message");$&&$.remove();let I=document.createElement("div");I.className="fix-message",I.style.cssText=`
      margin-top: 8px;
      padding: 8px 12px;
      border-radius: 6px;
      font-size: 12px;
    ...
    `,I.textContent=C;let R=w.querySelector(".issue-header");R?R.parentNode.insertBefore(I,R.nextSibling):w.appendChild(I),setTimeout(()=>{I.style.animation="slideOut 0.3s ease-out",setTimeout(()=>{I.parentNode&&I.remove()},300)},5e3)}function p(S){return S==null?"":String(S).replace(/[&<>"']/g,function(h){return{"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[h]})}function to(S){console.log("[showErrorModal] Called with message:",S);let C=document.getElementById("error-modal-overlay");C&&(console.log("[showErrorModal] Removing existing modal"),C.remove());let h=document.createElement("div");h.className="modal-overlay",h.id="error-modal-overlay",console.log("[showErrorModal] Created overlay element");let w=document.createElement("div");w.className="modal-dialog",w.style.maxWidth="450px";let $=String(S||"").replace(/^[❌⚠️✅]\s*/,"").trim();w.innerHTML=`
    ...
    `,h.appendChild(w),document.body.appendChild(h),console.log("[showErrorModal] Appended overlay to body, overlay visible:",h.offsetParent!==null),h.style.display="flex",h.style.visibility="visible",h.style.opacity="1";let I=w.querySelector("#error-modal-ok-btn"),R=w.querySelector(".modal-close");if(console.log("[showErrorModal] Found buttons:",{okBtn:!!I,closeBtn:!!R}),!I){console.error("[showErrorModal] OK button not found!");return}let _=()=>{h.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{h.parentNode&&h.remove()},200)};I.onclick=_,R&&(R.onclick=_),h.onclick=O=>{O.target===h&&_()},setTimeout(()=>{I.focus()},100),console.log("[showErrorModal] Modal setup complete")}function dt(S){if(!S)return 0;let C=String(S).replace("#","");if(C.length<6)return 0;let h=parseInt(C.substring(0,2),16),w=parseInt(C.substring(2,4),16),$=parseInt(C.substring(4,6),16);return isNaN(h)||isNaN(w)||isNaN($)?0:(h*299+w*587+$*114)/1e3}function qt(S,C){let h=String(S||"").replace("#",""),w=String(C||"").replace("#","");if(h.length<6||w.length<6)return 1/0;let $=parseInt(h.substring(0,2),16),I=parseInt(h.substring(2,4),16),R=parseInt(h.substring(4,6),16),_=parseInt(w.substring(0,2),16),O=parseInt(w.substring(2,4),16),L=parseInt(w.substring(4,6),16);if([$,I,R,_,O,L].some(G=>isNaN(G)))return 1/0;let U=_-$,V=O-I,B=L-R;return Math.sqrt(U*U+V*V+B*B)}function cn(S){let C=String(S||"").replace("#","");if(C.length<6)return 0;let h=parseInt(C.substring(0,2),16)/255,w=parseInt(C.substring(2,4),16)/255,$=parseInt(C.substring(4,6),16)/255;if(isNaN(h)||isNaN(w)||isNaN($))return 0;let I=h<=.03928?h/12.92:Math.pow((h+.055)/1.055,2.4),R=w<=.03928?w/12.92:Math.pow((w+.055)/1.055,2.4),_=$<=.03928?$/12.92:Math.pow(($+.055)/1.055,2.4);return .2126*I+.7152*R+.0722*_}function Pt(S,C){let h=cn(S),w=cn(C),$=Math.max(h,w),I=Math.min(h,w);return($+.05)/(I+.05)}function St(S){if(!S||!S.message)return null;let h=(S.message||"").match(/Color (#[0-9A-Fa-f]{6})/),w=h?h[1].toUpperCase():null;if(!w)return null;let $=document.getElementById("color-scale");if(!$||!$.value.trim())return null;let I=$.value.split(",").map(O=>O.trim().toUpperCase()).filter(O=>O&&O.startsWith("#"));if(I.length===0)return null;let R=null,_=1/0;return I.forEach(O=>{let L=qt(w,O);L<_&&(_=L,R=O)}),_>100?null:R}function kt(S){if(!S||!S.message)return null;let h=(S.message||"").match(/\((\d+)px\)/);if(!h)return null;let w=parseInt(h[1],10);if(isNaN(w))return null;let $=document.getElementById("spacing-scale");if(!$||!$.value.trim())return null;let I=$.value.split(",").map(L=>parseInt(L.trim(),10)).filter(L=>!isNaN(L)&&L>=0).sort((L,U)=>L-U);if(I.length===0)return null;let R=null,_=1/0;I.forEach(L=>{let U=Math.abs(L-w);U<_&&(_=U,R=L)});let O=Math.max(w*.2,10);return _>O?null:R}function al(S,C){if(!S||S.length===0){alert("No typography styles available. Please add styles in Typography Settings.");return}let h=document.createElement("div");h.className="modal-overlay",h.style.zIndex="10001";let w=document.createElement("div");w.className="modal-dialog",w.style.maxWidth="300px",w.style.padding="0",w.innerHTML=`
      <div class="modal-header" style="padding: 16px;">
        <button class="modal-close" aria-label="Close">\xD7</button>
        <h2 class="modal-title" style="font-size: 14px;">Choose Typography Style</h2>
      </div>
      <div style="max-height: 300px; overflow-y: auto;">
        ${S.map(I=>{let R=p(I.name||""),_=p(I.fontFamily||""),O=p(I.fontSize||""),L=p(I.fontWeight||"");return`
    ...
    `,h.appendChild(w),document.body.appendChild(h);let $=()=>{h.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{h.parentNode&&h.remove()},200)};w.querySelector(".modal-close").onclick=$,h.onclick=I=>{I.target===h&&$()},w.querySelectorAll(".style-dropdown-item").forEach(I=>{I.onclick=R=>{R.preventDefault(),R.stopPropagation();let _=parseInt(I.getAttribute("data-style-id"),10),O=S.find(L=>L.id===_);O&&C&&(C(O),$())}})}function dn(S,C){if(!S||!S.bestMatch){console.error("showTypographyFixModal: missing issue or bestMatch");return}let h=S.nodeProps||{},w=S.bestMatch,$=h.fontFamily||"",I=h.fontSize!==null&&h.fontSize!==void 0?h.fontSize:"",R=h.fontWeight||"",_=h.lineHeight||"",O=h.letterSpacing!==null&&h.letterSpacing!==void 0?h.letterSpacing:"",L=$,U=I,V=R,B=_,G=O;w.differences&&w.differences.forEach(Y=>{Y.property==="Font Family"&&Y.expected?L=Y.expected:Y.property==="Font Size"&&Y.expected?U=Y.expected.replace("px",""):Y.property==="Font Weight"&&Y.expected?V=Y.expected:Y.property==="Line Height"&&Y.expected?B=Y.expected:Y.property==="Letter Spacing"&&Y.expected&&(G=Y.expected)});let ae=document.createElement("div");ae.className="modal-overlay",ae.id="typography-fix-modal-overlay";let Z=document.createElement("div");Z.className="modal-dialog",Z.style.maxWidth="500px",Z.innerHTML=`
      <div class="modal-header">
        <button class="modal-close" aria-label="Close">\xD7</button>
        <h2 class="modal-title">Fix Typography Style</h2>
        <p class="modal-subtitle">Node: ${p(S.nodeName||"Unnamed")} \u2192 Suggested: ${p(w.name)}</p>
      </div>
      <div class="modal-body">
        <div style="margin-bottom: 16px;">
          ...
          <div style="margin-bottom: 12px;">
            ...
            <input type="text" class="modal-input" id="fix-font-family" value="${p(L)}" style="width: 100%;" />
          </div>

          <div style="margin-bottom: 12px;">
            <label style="display: block; font-size: 12px; font-weight: 600; color: #333; margin-bottom: 4px;">Font Size (px)</label>
            <input type="number" class="modal-input" id="fix-font-size" value="${p(U)}" style="width: 100%;" />
          </div>
          ...
          <div style="margin-bottom: 12px;">
            ...
            <input type="text" class="modal-input" id="fix-line-height" value="${p(B)}" style="width: 100%;" placeholder="e.g. 120%, 24px, auto" />
          </div>
    ...
    `,ae.appendChild(Z),document.body.appendChild(ae);let oe=Z.querySelector("#fix-font-family"),ne=Z.querySelector("#fix-font-size"),me=Z.querySelector("#fix-font-weight"),ee=Z.querySelector("#fix-line-height"),le=Z.querySelector("#fix-letter-spacing"),H=Z.querySelector("#choose-typo-style-btn"),pe=Z.querySelector("#typography-fix-modal-cancel-btn"),J=Z.querySelector("#typography-fix-modal-apply-btn"),ue=Z.querySelector(".modal-close");setTimeout(()=>{oe.focus()},100);let k=()=>{ae.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{ae.parentNode&&ae.remove()},200)};pe.onclick=k,ue.onclick=k,ae.onclick=Y=>{Y.target===ae&&k()},H.onclick=()=>{al(C,Y=>{Y&&(oe.value=Y.fontFamily||"",ne.value=Y.fontSize||"",me.value=Y.fontWeight||"Regular",ee.value=Y.lineHeight||"",le.value=Y.letterSpacing||"0")})},J.onclick=()=>{let Y={fontFamily:oe.value.trim(),fontSize:ne.value.trim(),fontWeight:me.value,lineHeight:ee.value.trim(),letterSpacing:le.value.trim()};if(!Y.fontFamily){oe.focus(),oe.style.borderColor="#ff3b30",setTimeout(()=>{oe.style.borderColor="#0071e3"},2e3);return}if(!Y.fontSize||isNaN(parseFloat(Y.fontSize))){ne.focus(),ne.style.borderColor="#ff3b30",setTimeout(()=>{ne.style.borderColor="#0071e3"},2e3);return}k(),re(S.id,"\u23F3 Fixing...",!0),parent.postMessage({pluginMessage:{type:"fix-issue",issue:S,fixData:Y}},"*")}}function rl(S,C){if(S===C)return 100;let h=100,w=Math.abs(S-C);return Math.max(0,Math.round((1-w/h)*100))}function pn(S,C,h,w){let $=document.createElement("div");$.className="modal-overlay",$.id="spacing-picker-modal-overlay";let I=document.createElement("div");I.className="modal-dialog",I.style.maxWidth="400px";let R=w.map(B=>`
        <div class="spacing-picker-item" data-value="${B}" style="
          ...
          background: white;
          transition: all 0.2s;
        " onmouseover="this.style.borderColor='#0071e3'; this.style.boxShadow='0 2px 8px rgba(0,113,227,0.2)'" onmouseout="this.style.borderColor='${h===B?"#0071e3":"#ddd"}'; this.style.boxShadow='none'">
    ...
    `,$.appendChild(I),document.body.appendChild($);let O=I.querySelector("#spacing-picker-modal-cancel-btn"),L=I.querySelector(".modal-close"),U=I.querySelectorAll(".spacing-picker-item"),V=()=>{$.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{$.parentNode&&$.remove()},200)};O.onclick=V,L.onclick=V,$.onclick=B=>{B.target===$&&V()},U.forEach(B=>{B.onclick=G=>{G.preventDefault(),G.stopPropagation();let ae=parseInt(B.getAttribute("data-value"),10);V(),Rt(S,C,h,ae)}})}function Rt(S,C,h,w,$=[],I={}){let{onApply:R,onIgnore:_,onCancel:O,showIgnore:L=!1,progress:U}=I,V=String(C||"").replace(/([A-Z])/g," $1").replace(/^./,X=>X.toUpperCase()).trim(),G=($.length>0?$:[0,4,8,12,16,24,32,40,48,64,72,80,88,96]).map(X=>({value:X,similarity:rl(h,X),diff:Math.abs(h-X)})).sort((X,fe)=>w!==void 0&&X.value===w?-1:w!==void 0&&fe.value===w?1:X.diff-fe.diff).slice(0,5);if(G.length===0){alert("No spacing values available");return}let ae=G[0].value,Z=(X,fe)=>{let Se=X.value!==h;return`
      ...
      ">
        ...
        <div style="flex: 1;">
          ...
          <div style="font-size: 10px; color: ${Se?"#721c24":"#155724"};">
            ${Se?`\u26A0 \u0394${X.value>h?"+":""}${X.value-h}px`:"\u2713 Same"}
          </div>
        </div>
    ...
    `},oe=document.createElement("div");oe.className="modal-overlay",oe.id="spacing-fix-confirm-modal-overlay";let ne=document.createElement("div");ne.className="modal-dialog",ne.style.maxWidth="420px";let me=U?`<div style="margin-bottom: 12px; padding: 8px 12px; background: #e3f2fd; border-radius: 6px; font-size: 13px; color: #1976d2; font-weight: 600;">Progress: ${U.current}/${U.total}</div>`:"",ee=G.map((X,fe)=>Z(X,fe===0)).join("");ne.innerHTML=`
      <div class="modal-header">
        ...
        <p class="modal-subtitle">Node: ${p(S.nodeName||"Unnamed")} - ${p(V)}</p>
      </div>
      ${me}
      <div class="modal-body">
    ...
    `,oe.appendChild(ne),document.body.appendChild(oe);let le=ne.querySelector("#spacing-fix-confirm-cancel-btn"),H=ne.querySelector("#spacing-fix-confirm-apply-btn"),pe=ne.querySelector("#spacing-fix-ignore-btn"),J=ne.querySelector(".modal-close"),ue=ne.querySelector("#spacing-options-container"),k=X=>{ae=X,ue.querySelectorAll(".spacing-option-item").forEach(Se=>{let Q=parseInt(Se.getAttribute("data-value"),10)===X;Se.style.border=Q?"2px solid #0071e3":"2px solid #e0e0e0",Se.style.background=Q?"#e3f2fd":"white";let nt=Se.querySelector('input[type="radio"]');nt&&(nt.checked=Q)})};(()=>{ue.querySelectorAll(".spacing-option-item").forEach(fe=>{fe.onclick=Se=>{Se.preventDefault();let je=parseInt(fe.getAttribute("data-value"),10);k(je)}})})();let te=()=>{oe.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{oe.parentNode&&oe.remove()},200)};le.onclick=()=>{te(),O&&typeof O=="function"&&O()},J.onclick=()=>{te(),O&&typeof O=="function"&&O()},oe.onclick=X=>{X.target===oe&&(te(),O&&typeof O=="function"&&O())},H.onclick=()=>{te(),re(S.id,"\u23F3 Fixing spacing...",!0),parent.postMessage({pluginMessage:{type:"fix-spacing-issue",issue:S,propertyName:C,value:ae}},"*"),R&&typeof R=="function"&&R()},pe&&(pe.onclick=()=>{te(),_&&typeof _=="function"&&_()})}function wt(S){try{let C=parseInt(S.slice(1,3),16),h=parseInt(S.slice(3,5),16),w=parseInt(S.slice(5,7),16);return(C*299+h*587+w*114)/1e3>128?"#000000":"#ffffff"}catch(C){return"#000000"}}function cl(S,C){let h=S.replace("#",""),w=C.replace("#",""),$=parseInt(h.substr(0,2),16),I=parseInt(h.substr(2,2),16),R=parseInt(h.substr(4,2),16),_=parseInt(w.substr(0,2),16),O=parseInt(w.substr(2,2),16),L=parseInt(w.substr(4,2),16);return Math.sqrt(Math.pow($-_,2)+Math.pow(I-O,2)+Math.pow(R-L,2))}function dl(S,C){let w=cl(S,C);return Math.round((1-w/441.67)*100)}function oo(S,C,h,w="",$=""){return wt(S),`
      <div class="color-picker-item" data-color="${p(S)}" style="
        padding: 7px;
        margin-bottom: 8px;
        border: 1px solid ${h};
        border-radius: 8px;
    ...
    `}function un(S,C,h,w={}){if(!C){let B=(S.message||"").match(/Color (#[0-9A-Fa-f]{6})/);C=B?B[1].toUpperCase():null}if(!C){alert("Cannot determine current color from issue message");return}let $=document.createElement("div");$.className="modal-overlay",$.id="color-picker-modal-overlay";let I=document.createElement("div");I.className="modal-dialog",I.style.maxWidth="400px";let R=h.map(V=>{let B=w[V]||"";return oo(V,B||V,C===V?"#0071e3":"#ddd","",C===V?"Current":"")}).join("");I.innerHTML=`
      <div class="modal-header">
        <button class="modal-close" aria-label="Close">\xD7</button>
        <h2 class="modal-title">Choose Color</h2>
        <p class="modal-subtitle">Node: ${p(S.nodeName||"Unnamed")}</p>
      </div>
      <div class="modal-body">
        <div style="margin-bottom: 12px; padding: 12px; background: #f5f5f5; border-radius: 6px;">
          <div style="font-size: 12px; color: #666; margin-bottom: 4px;">Current Color:</div>
          <div style="display: flex; align-items: center; gap: 8px;">
            <div style="width: 32px; height: 32px; border-radius: 4px; background: ${p(C)}; border: 1px solid #ddd;"></div>
            <div style="font-family: 'SF Mono', Monaco, monospace; font-size: 11px; font-weight: 600;">${p(C)}</div>
          </div>
        </div>
        <div style="max-height: 300px; overflow-y: auto;">
          ${R}
    ...
    `,$.appendChild(I),document.body.appendChild($);let _=I.querySelector("#color-picker-modal-cancel-btn"),O=I.querySelector(".modal-close"),L=I.querySelectorAll(".color-picker-item"),U=()=>{$.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{$.parentNode&&$.remove()},200)};_.onclick=U,O.onclick=U,$.onclick=V=>{V.target===$&&U()},L.forEach(V=>{V.onclick=B=>{B.preventDefault(),B.stopPropagation();let G=V.getAttribute("data-color");U(),Dt(S,C,G,w)}})}function Dt(S,C,h,w={},$=[],I={}){let{onApply:R,onIgnore:_,onCancel:O,showIgnore:L=!1,progress:U}=I,V=$.length>0?$:Object.keys(w);V.length===0&&h&&(V=[h]);let B=V.map(te=>({color:te,name:w[te]||te,similarity:dl(C,te)})).sort((te,X)=>h&&te.color===h?-1:h&&X.color===h?1:X.similarity-te.similarity).slice(0,5);if(B.length===0){alert("No colors available");return}let G=B[0].color,ae=(te,X)=>`
      <div class="color-option-item" data-color="${p(te.color)}" style="
        padding: 10px 12px;
        margin-bottom: 6px;
      ...
      ">
        ...
        <div style="
          ...
          height: 36px;
          border-radius: 6px;
          background: ${p(te.color)};
          border: 1px solid ${X?"#0071e3":"#ddd"};
          flex-shrink: 0;
        "></div>
        <div style="flex: 1;">
          <div style="font-weight: 600; font-size: 13px; color: #333;">${p(te.name)}</div>
          <div style="font-size: 11px; color: #666; font-family: 'SF Mono', Monaco, monospace;">${p(te.color)}</div>
        </div>
      ...
      <div class="modal-body">
        <div style="margin-bottom: 10px; padding: 10px; background: #f8f9fa; border-radius: 6px; display: flex; align-items: center; gap: 10px;">
          ...
          <div>
            ...
            <div style="font-size: 12px; font-weight: 600; font-family: 'SF Mono', Monaco, monospace;">${p(C)}</div>
          </div>
        </div>
        <div style="font-size: 12px; font-weight: 600; color: #333; margin-bottom: 8px;">
...
<body>
  <div class="container">
    ...
    <div class="section">
      ...
      </div>`;for(let[B,G]of Object.entries(V)){let ae=p(O(B));L+=`
      <div class="issue-group collapsed" data-type="${B}" data-label="${ae}">
        ...
        <div class="issue-group-content">`,G.forEach(Z=>{L+=`
          <div class="issue ${Z.severity}">
            ...
            <div class="issue-message">${p(Z.message)}</div>
            ${Z.nodeName?`<div class="issue-node">Node: ${p(Z.nodeName)}</div>`:""}
          </div>`}),L+=`
        </div>
      </div>`}L+=`
    </div>`}if($){L+=`
        ...
        <div class="issue-group-content">`,!B.values||B.values.length===0){L+=`
          ...
          <div class="token-item">
            <div class="token-value">
              <span class="token-color-preview" style="background-color: ${p(ae)}"></span>
              ${p(ae)}
              ${oe?`<span class="token-color-type">${p(oe)}</span>`:""}
            </div>
            ${Z>1?`<div class="token-node-count">Used in ${Z} nodes</div>`:""}
          </div>`;else if(V==="gradients")L+=`
          <div class="token-item">
            <div class="token-value">
              <span class="token-color-preview" style="background: ${p(ae)}"></span>
              ${p(ae)}
              ${oe?`<span class="token-color-type">${p(oe)}</span>`:""}
            </div>
            ${Z>1?`<div class="token-node-count">Used in ${Z} nodes</div>`:""}
          </div>`;else{let ne="";if(V==="fontWeight"){let me=Array.isArray(G.fontFamilies)?G.fontFamilies:null;if(!me){let ee={};(Array.isArray(G.nodes)?G.nodes:[]).forEach(H=>{let pe=H&&H.fontFamily?String(H.fontFamily):"Unknown";ee[pe]=(ee[pe]||0)+1}),me=Object.entries(ee).map(([H,pe])=>({family:H,count:pe})).sort((H,pe)=>pe.count-H.count||H.family.localeCompare(pe.family))}Array.isArray(me)&&me.length>0&&(ne=`<div class="token-node-count" style="margin-top: 6px;">Font-family:<br/>${me.map(le=>`${p(le.family)} (${le.count})`).join("<br/>")}</div>`)}L+=`
          <div class="token-item">
            <div class="token-value">${p(String(ae))}</div>
            ${Z>1?`<div class="token-node-count">Used in ${Z} nodes</div>`:""}
            ${ne}
          </div>`}}),L+=`
          </div>
  ...
  <\/script>
...
</html>`,L}function pl(){let S=new Date,C=S.getFullYear(),h=String(S.getMonth()+1).padStart(2,"0"),w=String(S.getDate()).padStart(2,"0"),$=String(S.getHours()).padStart(2,"0"),I=String(S.getMinutes()).padStart(2,"0"),R=String(S.getSeconds()).padStart(2,"0");return`design-review-report-${C}-${h}-${w}-${$}-${I}-${R}`}function ul(S,C){if(!S||!Array.isArray(S.colors))return S;let h=C||{};return ot(Ke({},S),{colors:S.colors.map(w=>ot(Ke({},w),{name:h[String(w.value).toUpperCase()]||"No name"}))})}function gn({format:S,reportData:C,getTypeDisplayName:h,filenameBase:w,colorNameMap:$}={}){var _,O;let I=C||{};if(!I.issues&&!I.tokens){alert("No data to export!");return}let R=w||pl();if(S==="html"){let L=no({reportData:I,getTypeDisplayName:h}),U=new Blob([L],{type:"text/html"}),V=URL.createObjectURL(U),B=document.createElement("a");B.href=V,B.download=`${R}.html`,B.click(),URL.revokeObjectURL(V);return}if(S==="pdf"){let L=no({reportData:I,getTypeDisplayName:h});try{let U=window.open("","_blank");if(!U){alert("Popup blocked. Downloading HTML - you can open the file and select Print to create PDF.");let V=new Blob([L],{type:"text/html"}),B=URL.createObjectURL(V),G=document.createElement("a");G.href=B,G.download=`${R}.html`,G.click(),URL.revokeObjectURL(B);return}U.document.open(),U.document.write(L),U.document.close(),U.onload=()=>{setTimeout(()=>{U.print()},250)},setTimeout(()=>{U.document&&U.document.readyState==="complete"&&U.print()},500)}catch(U){console.error("Error opening print window:",U),alert("Cannot open print window. Downloading HTML - you can open the file and select Print to create PDF.");let V=new Blob([L],{type:"text/html"}),B=URL.createObjectURL(V),G=document.createElement("a");G.href=B,G.download=`${R}.html`,G.click(),URL.revokeObjectURL(B)}return}if(S==="json"){console.log("Export JSON - colorNameMap:",$),console.log("Export JSON - tokens.colors:",(_=I.tokens)==null?void 0:_.colors);let L=ot(Ke({},I),{tokens:ul(I.tokens,$)});console.log("Export JSON - enriched colors:",(O=L.tokens)==null?void 0:O.colors);let U=JSON.stringify(L,null,2),V=new Blob([U],{type:"application/json"}),B=URL.createObjectURL(V),G=document.createElement("a");G.href=B,G.download=`${R}.json`,G.click(),URL.revokeObjectURL(B);return}}function mn({maxHistory:S=10,postPluginMessage:C,getCurrentReportData:h,setIsViewingTokens:w,renderResults:$,renderTokens:I}={}){let R=[],_=!1,O=ee=>{try{typeof C=="function"&&C(ee)}catch(le){console.error("scanHistory postPluginMessage error:",le)}};function L(ee){R=(Array.isArray(ee)?ee:[]).slice(0,S)}function U(){R=[],_=!1}function V(){return R||[]}function B(ee,le,H,pe){try{if(le==="issues"&&(!H.issues||H.issues.length===0)){console.log("Skipping save - no issues data");return}if(le==="tokens"&&(!H.tokens||Object.keys(H.tokens).length===0)){console.log("Skipping save - no tokens data");return}let J={id:Date.now().toString(),mode:ee,type:le,timestamp:new Date().toISOString(),context:pe||null,data:{issues:H.issues||null,tokens:H.tokens||null,issuesCount:H.issues?H.issues.length:0,tokensCount:H.tokens?Object.keys(H.tokens).reduce((k,Y)=>{var te;return k+(((te=H.tokens[Y])==null?void 0:te.length)||0)},0):0}};R.unshift(J),R=R.slice(0,S);let ue=document.getElementById("history-panel");ue&&ue.style.display!=="none"&&ne(),O({type:"save-history-entry",entry:J})}catch(J){console.error("Error saving scan history:",J)}}function G(){O({type:"get-history"})}function ae(){try{let ee=V();if(ee.length>0){let le=ee[0],H=document.querySelector(`input[name="scope"][value="${le.mode}"]`);H&&(H.checked=!0,console.log("Loaded last scan mode:",le.mode))}}catch(ee){console.error("Error loading last scan mode:",ee)}}function Z(){_||(ae(),_=!0)}function oe(ee){try{let le=new Date(ee),pe=new Date-le,J=Math.floor(pe/6e4),ue=Math.floor(pe/36e5),k=Math.floor(pe/864e5);return J<1?"Just now":J<60?`${J} minutes ago`:ue<24?`${ue} hours ago`:k<7?`${k} days ago`:le.toLocaleString("en-US",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"})}catch(le){return ee}}function ne(){let ee=document.getElementById("history-list");if(!ee){console.error("history-list element not found");return}let le=V();if(console.log("Rendering scan history:",le.length,"entries"),le.length===0){ee.innerHTML=`
              <div class="history-empty">
                <div class="icon">\u{1F4CB}</div>
                <p>No scan history</p>
                <p style="font-size: 11px; margin-top: 8px;">Scans will be saved automatically</p>
              </div>
            `;return}ee.innerHTML=le.map(H=>{var X,fe;let pe=oe(H.timestamp),J=H.mode==="page"?"Page":"Selection",ue=H.type==="issues"?"Issues":"Tokens",k=H.type==="issues"?"issues":"tokens",Y=H.context&&H.context.label?H.context.label:`${J} scan`,te="";if(H.type==="issues"){let Se=((X=H.data.issues)==null?void 0:X.filter(Q=>Q.severity==="error").length)||0,je=((fe=H.data.issues)==null?void 0:fe.filter(Q=>Q.severity==="warn").length)||0;te=`
                <span>\u274C ${Se} errors</span>
    ...
    `,i.appendChild(c),document.body.appendChild(i);let u=c.querySelector("#autolayout-fix-cancel-btn"),d=c.querySelector("#autolayout-fix-apply-btn"),b=c.querySelector("#autolayout-fix-ignore-btn"),y=c.querySelector(".modal-close"),f=()=>{i.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{i.parentNode&&i.remove()},200)};u.onclick=()=>{f(),n&&n()},y.onclick=()=>{f(),n&&n()},i.onclick=m=>{m.target===i&&(f(),n&&n())},b.onclick=()=>{f(),l&&l()},d.onclick=()=>{f(),re(e.id,"\u23F3 Enabling auto layout...",!0),parent.postMessage({pluginMessage:{type:"fix-autolayout-issue",issue:e}},"*"),o&&o()}}function Cn(e){return{action:"convert-group"}}function ro(e){if(!Cn(e)){alert("Cannot suggest fix for this group issue");return}In(e)}function In(e){let t=document.createElement("div");t.className="modal-overlay",t.id="group-fix-modal-overlay";let o=document.createElement("div");o.className="modal-dialog",o.style.maxWidth="450px",o.innerHTML=`
      <div class="modal-header">
        <button class="modal-close" aria-label="Close">\xD7</button>
    ...
    `,i.appendChild(c),document.body.appendChild(i);let u=c.querySelector("#group-fix-cancel-btn"),d=c.querySelector("#group-fix-apply-btn"),b=c.querySelector("#group-fix-ignore-btn"),y=c.querySelector(".modal-close"),f=()=>{i.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{i.parentNode&&i.remove()},200)};u.onclick=()=>{f(),n&&n()},y.onclick=()=>{f(),n&&n()},i.onclick=m=>{m.target===i&&(f(),n&&n())},b.onclick=()=>{f(),l&&l()},d.onclick=()=>{d.disabled=!0,d.textContent="Applying...",parent.postMessage({pluginMessage:{type:"fix-group-issue",issue:e}},"*"),f(),o&&o()}}function st(e){return!e||e.type!=="position"?null:{action:"fix-position"}}function ut(e){return!e||e.type!=="duplicate"&&e.type!=="component"?null:{action:"suggest-component"}}function gt(e){return!e||e.type!=="empty-frame"?null:{action:"fix-empty-frame"}}function co(e){if(!st(e)){alert("Cannot suggest fix for this position issue");return}En(e)}function po(e){if(!gt(e)){alert("Cannot suggest fix for this empty frame issue");return}zn(e)}function En(e){let t=e.message||"",o=t.match(/x:(-?\d+)/),l=t.match(/y:(-?\d+)/),n=o?parseInt(o[1],10):0,s=l?parseInt(l[1],10):0,a=document.createElement("div");a.className="modal-overlay",a.id="position-fix-confirm-modal-overlay";let i=document.createElement("div");i.className="modal-dialog",i.style.maxWidth="400px",i.innerHTML=`
      <div class="modal-header">
        <button class="modal-close" aria-label="Close">\xD7</button>
    ...
    `,a.appendChild(i),document.body.appendChild(a);let c=i.querySelector("#position-fix-confirm-cancel-btn"),u=i.querySelector("#position-fix-confirm-apply-btn"),d=i.querySelector(".modal-close"),b=()=>{a.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{a.parentNode&&a.remove()},200)};c.onclick=b,d.onclick=b,a.onclick=y=>{y.target===a&&b()},u.onclick=()=>{b(),re(e.id,"\u23F3 Fixing position...",!0),parent.postMessage({pluginMessage:{type:"fix-position-issue",issue:e}},"*")}}function Mn(e,t={}){let{onApply:o,onIgnore:l,onCancel:n,progress:s}=t,a=s?`<div style="margin-bottom: 12px; padding: 8px 12px; background: #e3f2fd; border-radius: 6px; font-size: 13px; color: #1976d2; font-weight: 600;">Progress: ${s.current}/${s.total}</div>`:"",i=e.message||"",c=i.match(/x:(-?\d+)/),u=i.match(/y:(-?\d+)/),d=c?parseInt(c[1],10):0,b=u?parseInt(u[1],10):0,y=document.createElement("div");y.className="modal-overlay",y.id="position-fix-confirm-modal-overlay";let f=document.createElement("div");f.className="modal-dialog",f.style.maxWidth="400px",f.innerHTML=`
      <div class="modal-header">
        <button class="modal-close" aria-label="Close">\xD7</button>
        <h2 class="modal-title">Fix Position Issue</h2>
        <p class="modal-subtitle">Node: ${p(e.nodeName||"Unnamed")}</p>
    ...
    `,y.appendChild(f),document.body.appendChild(y);let m=f.querySelector("#position-fix-cancel-btn"),g=f.querySelector("#position-fix-apply-btn"),E=f.querySelector("#position-fix-ignore-btn"),T=f.querySelector(".modal-close"),A=()=>{y.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{y.parentNode&&y.remove()},200)};m.onclick=()=>{A(),n&&n()},T.onclick=()=>{A(),n&&n()},y.onclick=N=>{N.target===y&&(A(),n&&n())},E.onclick=()=>{A(),l&&l()},g.onclick=()=>{A(),re(e.id,"\u23F3 Fixing position...",!0),parent.postMessage({pluginMessage:{type:"fix-position-issue",issue:e}},"*"),o&&o()}}function gl(e){uo(e)}function Nt(e){uo(e)}function uo(e){let t=document.createElement("div");t.className="modal-overlay",t.id="remove-layer-confirm-modal-overlay";let o=document.createElement("div");o.className="modal-dialog",o.style.maxWidth="400px",o.innerHTML=`
      <div class="modal-header">
        <button class="modal-close" aria-label="Close">\xD7</button>
    ...
    `,document.body.appendChild(a),a.appendChild(i);let c=i.querySelector("#empty-frame-fix-cancel-btn"),u=i.querySelector(".modal-close"),d=i.querySelector("#empty-frame-fix-apply-btn"),b=()=>{a.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{a.parentNode&&a.remove()},200)};c.onclick=b,u.onclick=b,a.onclick=y=>{y.target===a&&b()},d.onclick=()=>{d.disabled=!0,d.textContent="Applying...",parent.postMessage({pluginMessage:{type:"fix-empty-frame-issue",issue:e}},"*"),b()}}function Tn(e,t={}){let{onApply:o,onIgnore:l,onCancel:n,progress:s}=t,a=s?`<div style="margin-bottom: 12px; padding: 8px 12px; background: #e3f2fd; border-radius: 6px; font-size: 13px; color: #1976d2; font-weight: 600;">Progress: ${s.current}/${s.total}</div>`:"",i=e.message||"",c=i.includes("Empty frame"),u=i.includes("redundant"),d=u?"Remove Redundant Frame":"Remove Empty Frame",b=u?"This will remove the redundant frame and keep its single child. The child will inherit the frame's name if it was unnamed.":"This will remove the empty frame. If it has a child, the child will be kept.",y=document.createElement("div");y.className="modal-overlay",y.id="empty-frame-fix-modal-overlay";let f=document.createElement("div");f.className="modal-dialog",f.style.maxWidth="450px",f.innerHTML=`
      <div class="modal-header">
        <button class="modal-close" aria-label="Close">\xD7</button>
    ...
    `,t.appendChild(o),document.body.appendChild(t);let s=o.querySelector("#rename-cancel-btn"),a=o.querySelector("#rename-apply-btn"),i=o.querySelector(".modal-close"),c=o.querySelector("#rename-input");setTimeout(()=>{c.focus(),c.select()},100);let u=()=>{t.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{t.parentNode&&t.remove()},200)};s.onclick=u,i.onclick=u,t.onclick=d=>{d.target===t&&u()},c.addEventListener("keydown",d=>{d.key==="Enter"&&(d.preventDefault(),a.click())}),a.onclick=()=>{let d=c.value.trim();if(!d){alert("Please enter a name"),c.focus();return}if(/^(Frame|Group)\s*$/i.test(d)&&!confirm("The name still contains default naming (Frame/Group). Do you want to continue?")){c.focus();return}u(),re(e.id,"\u23F3 Renaming...",!0),parent.postMessage({pluginMessage:{type:"rename-node",issue:e,newName:d}},"*")}}function Fn(e){let t=document.createElement("div");t.className="modal-overlay",t.id="create-component-modal-overlay";let o=document.createElement("div");o.className="modal-dialog",o.style.maxWidth="400px",o.innerHTML=`
      <div class="modal-header">
        <button class="modal-close" aria-label="Close">\xD7</button>
    ...
    `,l.appendChild(n),document.body.appendChild(l);let s=n.querySelector("#component-apply-cancel-btn"),a=n.querySelector("#component-apply-apply-btn"),i=n.querySelector(".modal-close"),c=()=>{l.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{l.parentNode&&l.remove()},200)};s.onclick=c,i.onclick=c,l.onclick=u=>{u.target===l&&c()},a.onclick=()=>{c(),re(e.id,"\u23F3 Applying component...",!0),parent.postMessage({pluginMessage:{type:"apply-component-to-issue",issue:e,componentId:t.id}},"*")}}function it(e){if(!e||!e.fontSize)return null;let t=e.fontSize,o=14,l=document.getElementById("font-size-scale");if(l&&l.value.trim()){let n=l.value.split(",").map(s=>parseInt(s.trim(),10)).filter(s=>!isNaN(s)&&s>=o).sort((s,a)=>s-a);if(n.length>0){let s=null,a=1/0;n.forEach(c=>{if(c>=o){let u=Math.abs(c-t);u<a&&(a=u,s=c)}});let i=Math.max(t*.5,10);if(s&&a<=i)return s}}return t<o?o:null}function mt(e){if(!e||!e.textColor||!e.backgroundColor)return null;let t=e.textColor.toUpperCase(),o=e.backgroundColor.toUpperCase(),l=e.minContrast||4.5,n=document.getElementById("color-scale");if(!n||!n.value.trim())return null;let s=n.value.split(",").map(u=>u.trim().toUpperCase()).filter(u=>u&&u.startsWith("#"));if(s.length===0)return null;let a=null,i=0,c=1/0;return s.forEach(u=>{let d=Pt(u,o);if(d>=l){let b=qt(t,u);(d>i||d===i&&b<c)&&(i=d,a=u,c=b)}}),a}function bo(e,t){if(!t||t.length===0){alert("No text styles found in Figma. Please create text styles first.");return}let o=document.createElement("div");o.className="modal-overlay",o.id="text-style-picker-typography-modal-overlay";let l=document.createElement("div");l.className="modal-dialog",l.style.maxWidth="500px";let n=e.nodeProps||{},s=n.fontFamily||"Unknown",a=n.fontSize!==null&&n.fontSize!==void 0?n.fontSize:null,i=a!==null?`${a}px`:"Unknown",c=n.fontWeight||"Unknown",u=n.lineHeight||"Unknown",d=n.letterSpacing!==null&&n.letterSpacing!==void 0?n.letterSpacing:"Unknown",b=null;e.bestMatch&&e.bestMatch.name&&(b=e.bestMatch.name);let y=x=>{if(x==null||x==="Unknown")return"";let P=String(x).toLowerCase().trim(),v=P.match(/^([+-]?\d*\.?\d+)(px|%)?$/);if(v){let M=parseFloat(v[1]);if(Math.abs(M)<1e-6)return"0";let q=v[2]||"px";return`${Math.round(M*100)/100}${q}`}return P},f=x=>{let P=`picker_${y(s)}_${a}_${y(c)}_${y(u)}_${y(d)}_${x.id}`;return fe(P,()=>{let v=0;if(y(s)===y(x.fontFamily)&&(v+=25),a!==null&&x.fontSize){let M=Math.abs(a-x.fontSize);M===0?v+=30:M<=2?v+=25:M<=4?v+=20:M<=8&&(v+=10)}return y(c)===y(x.fontWeight)&&(v+=20),y(u)===y(x.lineHeight)&&(v+=15),y(d)===y(x.letterSpacing||"0")&&(v+=10),v})},m=[...t].sort((x,P)=>b===x.name?-1:b===P.name?1:f(P)-f(x)),g=(x,P)=>y(x)!==y(P),E=x=>{let P=b===x.name,v=f(x),M=g(s,x.fontFamily),q=g(i,`${x.fontSize}px`),se=g(c,x.fontWeight),he=g(u,x.lineHeight),Ee=g(d,x.letterSpacing||"0"),be="color: #155724;",F="color: #721c24; background: #f8d7da; padding: 1px 4px; border-radius: 3px; font-weight: 600;";return`
        <div class="style-picker-item" data-style-id="${x.id}" data-style-name="${p(x.name)}" data-font-size="${x.fontSize}" data-similarity="${v}" style="
          padding: 12px;
          margin-bottom: 8px;
        ...
        " onmouseover="this.style.borderColor='#0071e3'; this.style.boxShadow='0 2px 8px rgba(0,113,227,0.2)'" onmouseout="this.style.borderColor='${P?"#0071e3":"#ddd"}'; this.style.boxShadow='none'">
          <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 8px;">
            <div>
              <div style="font-weight: 600; font-size: 12px; color: #333;">${p(x.name)} ${P?"\u2B50":""}</div>
              <div style="font-size: 11px; color: #666; margin-top: 4px;">
                ${p(x.fontFamily)} ${x.fontSize}px ${p(x.fontWeight)}
              </div>
            </div>
            <div style="text-align: right;">
              ${P?'<div style="color: #0071e3; font-weight: 600; font-size: 11px;">Best Match</div>':""}
              <div style="color: #666; font-size: 10px; margin-top: 2px;">${v}% match</div>
            </div>
          </div>
          <div style="font-size: 11px; color: #666; padding-top: 8px; border-top: 1px solid #eee;">
            ...
            <div style="padding-left: 8px; line-height: 1.8;">
              \u2022 Font Family: <code style="${M?F:be}">${M?"\u26A0 ":"\u2713 "}${p(x.fontFamily)}</code><br>
              \u2022 Font Size: <code style="${q?F:be}">${q?"\u26A0 ":"\u2713 "}${x.fontSize}px</code><br>
              \u2022 Font Weight: <code style="${se?F:be}">${se?"\u26A0 ":"\u2713 "}${p(x.fontWeight)}</code><br>
              \u2022 Line Height: <code style="${he?F:be}">${he?"\u26A0 ":"\u2713 "}${p(x.lineHeight)}</code><br>
              \u2022 Letter Spacing: <code style="${Ee?F:be}">${Ee?"\u26A0 ":"\u2713 "}${p(x.letterSpacing||"0")}</code>
            </div>
          </div>
        </div>
      `},T=m.map(x=>E(x)).join("");l.innerHTML=`
      <div class="modal-header">
        <button class="modal-close" aria-label="Close">\xD7</button>
      ...
      <div class="modal-body">
        <div style="margin-bottom: 12px; padding: 12px; background: #f5f5f5; border-radius: 6px;">
          ...
          <div style="font-size: 11px; color: #666; padding-left: 8px; line-height: 1.6;">
            ...
            \u2022 Font Size: <code>${p(i)}</code><br>
            \u2022 Font Weight: <code>${p(c)}</code><br>
            \u2022 Line Height: <code>${p(u)}</code><br>
    ...
    `,o.appendChild(l),document.body.appendChild(o);let A=l.querySelector("#text-style-picker-typography-modal-cancel-btn"),N=l.querySelector(".modal-close"),j=l.querySelector("#style-search-input"),z=l.querySelector("#style-list-container"),D=()=>{o.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{o.parentNode&&o.remove()},200)};A.onclick=D,N.onclick=D,o.onclick=x=>{x.target===o&&D()},j.oninput=x=>{let P=x.target.value.toLowerCase().trim(),v=m.filter(M=>M.name.toLowerCase().includes(P));z.innerHTML=v.map(M=>E(M)).join(""),ye()};let ye=()=>{l.querySelectorAll(".style-picker-item").forEach(P=>{P.onclick=v=>{v.preventDefault(),v.stopPropagation();let M=P.getAttribute("data-style-id"),q=t.find(se=>se.id===M);q&&(o.style.display="none",Ln(e,q,o))}})};ye(),setTimeout(()=>{j.focus()},100)}function Ln(e,t,o){let l=document.createElement("div");l.className="modal-overlay",l.id="typography-style-confirm-modal-overlay";let n=document.createElement("div");n.className="modal-dialog",n.style.maxWidth="500px";let s=e.nodeProps||{},a=s.fontFamily||"Unknown",i=s.fontSize!==null&&s.fontSize!==void 0?`${s.fontSize}px`:"Unknown",c=s.fontWeight||"Unknown",u=s.lineHeight||"Unknown",d=s.letterSpacing!==null&&s.letterSpacing!==void 0?s.letterSpacing:"Unknown";n.innerHTML=`
      <div class="modal-header">
        <button class="modal-close" aria-label="Close">\xD7</button>
    ...
    `,l.appendChild(n),document.body.appendChild(l);let b=n.querySelector("#typography-style-confirm-cancel-btn"),y=n.querySelector("#typography-style-confirm-apply-btn"),f=n.querySelector(".modal-close"),m=()=>{l.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{l.parentNode&&l.remove(),o&&o.parentNode&&(o.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{o.parentNode&&o.remove()},200))},200)},g=()=>{l.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{l.parentNode&&l.remove(),o&&(o.style.display="flex")},200)};b.onclick=g,f.onclick=g,l.onclick=E=>{E.target===l&&g()},y.onclick=()=>{m(),re(e.id,"\u23F3 Applying style...",!0),$o(e,t)}}function Hn(e,t){let o=t.filter(d=>d.fontSize>=14);if(o.length===0){let d=it(e);d?Ft(e,e.fontSize||12,d,null,null):alert("No text styles found with fontSize >= 14px. Please add font sizes to Font Size input or create text styles in Figma.");return}let l=document.createElement("div");l.className="modal-overlay",l.id="text-style-picker-modal-overlay";let n=document.createElement("div");n.className="modal-dialog",n.style.maxWidth="400px";let s=o.map(d=>`
    ...
    `,l.appendChild(n),document.body.appendChild(l);let a=n.querySelector("#text-style-picker-modal-cancel-btn"),i=n.querySelector(".modal-close"),c=n.querySelectorAll(".style-picker-item"),u=()=>{l.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{l.parentNode&&l.remove()},200)};a.onclick=u,i.onclick=u,l.onclick=d=>{d.target===l&&u()},c.forEach(d=>{d.onclick=b=>{b.preventDefault(),b.stopPropagation();let y=d.getAttribute("data-style-id"),f=parseInt(d.getAttribute("data-font-size")),m=o.find(g=>g.id===y);m&&(l.style.display="none",Ft(e,e.fontSize||12,f,l,m))}})}function qn(e,t){let o=document.getElementById("color-scale"),l=o&&o.value.trim()?o.value.split(",").map(g=>g.trim().toUpperCase()).filter(g=>g&&g.startsWith("#")):[],n=[];if(t.filter(g=>g.source==="variable").forEach(g=>{n.push({source:"Variable",name:g.name,hex:g.hex,id:g.id,variable:g.variable})}),t.filter(g=>g.source==="style").forEach(g=>{n.push({source:"Style",name:g.name,hex:g.hex,id:g.id,style:g.style})}),l.forEach(g=>{let E=J[g]||g;n.push({source:"Input",name:E,hex:g,id:null})}),n.length===0){alert("No colors available. Please add colors to Color input or create color styles/variables in Figma.");return}let s=e.backgroundColor||"#FFFFFF",a=e.minContrast||4.5,i=e.textColor||"#000000",c=document.createElement("div");c.className="modal-overlay",c.id="contrast-color-picker-modal-overlay";let u=document.createElement("div");u.className="modal-dialog",u.style.maxWidth="400px";let d=n.map(g=>{let E=Pt(g.hex,s),T=E>=a,A=T?"#22c55e":"#ddd",N=`${p(g.name)} <span style="font-size: 11px; color: #666;">(${p(g.source)})</span>`,j=`<span style="color: ${T?"#22c55e":"#ef4444"};">Contrast: ${E.toFixed(2)}:1 ${T?"\u2713":"\u2717"} (need >= ${a}:1)</span>`;return oo(g.hex,N,A,j)}).join("");u.innerHTML=`
      <div class="modal-header">
        <button class="modal-close" aria-label="Close">\xD7</button>
        <h2 class="modal-title">Choose Color</h2>
      ...
      <div class="modal-body">
        <div style="margin-bottom: 12px; padding: 12px; background: #f5f5f5; border-radius: 6px;">
          ...
          <div style="display: flex; align-items: center; gap: 8px;">
            <div style="width: 32px; height: 32px; border-radius: 4px; background: ${p(i)}; border: 1px solid #ddd;"></div>
            <div style="font-family: 'SF Mono', Monaco, monospace; font-size: 11px; font-weight: 600;">${p(i)}</div>
    ...
    `,c.appendChild(u),document.body.appendChild(c);let b=u.querySelector("#contrast-color-picker-modal-cancel-btn"),y=u.querySelector(".modal-close"),f=u.querySelectorAll(".color-picker-item"),m=()=>{c.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{c.parentNode&&c.remove()},200)};b.onclick=m,y.onclick=m,c.onclick=g=>{g.target===c&&m()},f.forEach(g=>{g.onclick=E=>{E.preventDefault(),E.stopPropagation();let T=g.getAttribute("data-color");c.style.display="none",So(e,i,T,c)}})}function xo(e){parent.postMessage({pluginMessage:{type:"get-figma-text-styles",issueId:e.id}},"*"),window.pendingTextSizeIssue=e}function jt(e){parent.postMessage({pluginMessage:{type:"get-figma-text-styles",issueId:e.id}},"*"),window.pendingSuggestTextSizeIssue=e}function Ft(e,t,o,l,n){let s=(v,M)=>{if(v===M)return 100;let q=20,se=Math.abs(v-M);return Math.max(0,Math.round((1-se/q)*100))},i=[14,16,18,20,24,28,32,36,40,48].map(v=>({size:v,similarity:s(t,v),diff:Math.abs(t-v)})).sort((v,M)=>o!==void 0&&v.size===o?-1:o!==void 0&&M.size===o?1:v.diff-M.diff).slice(0,5),c=i.length>0?i[0].size:o,u=(v,M)=>{let q=v.size>=14;return`
        <div class="text-size-option-item" data-size="${v.size}" style="
          padding: 6px 8px;
          margin-bottom: 6px;
          border: 1px solid ${M?"#0071e3":"#e0e0e0"};
          border-radius: 8px;
          cursor: pointer;
          ...
          transition: all 0.15s;
        ">
          <input type="radio" name="text-size-option" ${M?"checked":""} style="margin: 0; cursor: pointer; display: none;" />
          <div style="
            width: 40px;
            height: 40px;
            border-radius: 6px;
            background: ${M?"#e3f2fd":"#f0f0f0"};
            border: 1px solid ${M?"#0071e3":"#ddd"};
            display: flex;
            align-items: center;
            justify-content: center;
            font-size: 11px;
          ...
          ">${v.size}</div>
          <div style="flex: 1;">
            <div style="font-weight: 600; font-size: 11px; color: #333;">${v.size}px</div>
            <div style="font-size: 10px; color: ${q?"#155724":"#721c24"};">
              ${q?"\u2713 ADA compliant":"\u26A0 Below minimum"}
            </div>
          </div>
          <span style="font-size: 11px; color: #666; background: #f0f0f0; padding: 2px 8px; border-radius: 10px;">${v.similarity}%</span>
        </div>
      `},d=document.createElement("div");d.className="modal-overlay",d.id="text-size-fix-confirm-modal-overlay";let b=document.createElement("div");b.className="modal-dialog",b.style.maxWidth="420px";let y=window.pendingTextSizeFixAllCallbacks||null,f=y&&y.progress?`<div style="margin-bottom: 12px; padding: 8px 12px; background: #e3f2fd; border-radius: 6px; font-size: 13px; color: #1976d2; font-weight: 600;">Progress: ${y.progress.current}/${y.progress.total}</div>`:"",m=n?`
    ...
    `:"",g=i.map((v,M)=>u(v,M===0)).join("");b.innerHTML=`
      <div class="modal-header">
        <button class="modal-close" aria-label="Close">\xD7</button>
    ...
    `,d.appendChild(b),document.body.appendChild(d);let E=b.querySelector("#text-size-fix-confirm-cancel-btn"),T=b.querySelector("#text-size-fix-confirm-apply-btn"),A=b.querySelector("#text-size-fix-ignore-btn"),N=b.querySelector(".modal-close"),j=b.querySelector("#text-size-options-container"),z=y,D=v=>{c=v,j.querySelectorAll(".text-size-option-item").forEach(q=>{let he=parseInt(q.getAttribute("data-size"),10)===v;q.style.border=he?"2px solid #0071e3":"2px solid #e0e0e0",q.style.background=he?"#e3f2fd":"white";let Ee=q.querySelector('input[type="radio"]');Ee&&(Ee.checked=he)})};(()=>{j.querySelectorAll(".text-size-option-item").forEach(M=>{M.onclick=q=>{q.preventDefault();let se=parseInt(M.getAttribute("data-size"),10);D(se)}})})();let x=()=>{d.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{d.parentNode&&d.remove(),l&&(l.style.display="block")},200)},P=()=>{d.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{d.parentNode&&d.remove(),l&&l.parentNode&&(l.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{l.parentNode&&l.remove()},200))},200)};E.onclick=()=>{z&&z.onCancel?(P(),window.pendingTextSizeFixAllCallbacks=null,z.onCancel()):x()},N.onclick=()=>{z&&z.onCancel?(P(),window.pendingTextSizeFixAllCallbacks=null,z.onCancel()):x()},d.onclick=v=>{v.target===d&&(z&&z.onCancel?(P(),window.pendingTextSizeFixAllCallbacks=null,z.onCancel()):x())},A&&(A.onclick=()=>{P(),z&&z.onIgnore&&(window.pendingTextSizeFixAllCallbacks=null,z.onIgnore())}),T.onclick=()=>{P(),re(e.id,"\u23F3 Fixing text size...",!0),n&&n.id?parent.postMessage({pluginMessage:{type:"apply-figma-text-style",issue:e,styleId:n.id,styleName:n.name}},"*"):parent.postMessage({pluginMessage:{type:"fix-text-size-issue",issue:e,fontSize:c}},"*"),z&&z.onApply&&(window.pendingTextSizeFixAllCallbacks=null,z.onApply())}}function vo(e){parent.postMessage({pluginMessage:{type:"get-contrast-colors",issue:e}},"*"),window.pendingContrastIssue=e}function Ot(e){let t=mt(e);if(!t){alert("No suitable color found that passes contrast requirements");return}So(e,e.textColor,t,null)}function So(e,t,o,l){let n=e.backgroundColor||"#FFFFFF",s=e.minContrast||4.5,a=(v,M)=>{let q=v.replace("#",""),se=M.replace("#",""),he=parseInt(q.substr(0,2),16),Ee=parseInt(q.substr(2,2),16),be=parseInt(q.substr(4,2),16),F=parseInt(se.substr(0,2),16),ce=parseInt(se.substr(2,2),16),K=parseInt(se.substr(4,2),16);return Math.sqrt(Math.pow(he-F,2)+Math.pow(Ee-ce,2)+Math.pow(be-K,2))},i=(v,M)=>{let se=a(v,M);return Math.round((1-se/441.67)*100)},u=Object.keys(J).map(v=>{let M=Pt(v,n);return{color:v,name:J[v]||v,contrast:M,passes:M>=s,similarity:i(t,v),distance:a(t,v)}}).filter(v=>v.passes).sort((v,M)=>o&&v.color===o?-1:o&&M.color===o?1:v.distance-M.distance).slice(0,5),d=u.length>0?u[0].color:o,b=(v,M)=>`
        <div class="contrast-color-option-item" data-color="${p(v.color)}" style="
          padding: 10px 12px;
          margin-bottom: 6px;
          border: 1px solid ${M?"#0071e3":"#e0e0e0"};
          border-radius: 8px;
        ...
        ">
          ...
          <div style="
            ...
            height: 36px;
            border-radius: 6px;
            background: ${p(v.color)};
            border: 1px solid ${M?"#0071e3":"#ddd"};
            flex-shrink: 0;
          "></div>
          <div style="flex: 1;">
            <div style="font-weight: 600; font-size: 13px; color: #333;">${p(v.name)}</div>
            <div style="font-size: 10px; color: #666; font-family: 'SF Mono', Monaco, monospace;">${p(v.color)}</div>
            <div style="font-size: 10px; color: #22c55e; margin-top: 2px;">\u2713 ${v.contrast.toFixed(2)}:1</div>
          </div>
          <span style="font-size: 11px; color: #666; background: #f0f0f0; padding: 2px 8px; border-radius: 10px;">${v.similarity}%</span>
        </div>
      `,y=window.pendingContrastFixAllCallbacks||null,f=y&&y.progress?`<div style="margin-bottom: 12px; padding: 8px 12px; background: #e3f2fd; border-radius: 6px; font-size: 13px; color: #1976d2; font-weight: 600;">Progress: ${y.progress.current}/${y.progress.total}</div>`:"",m=document.createElement("div");m.className="modal-overlay",m.id="contrast-fix-confirm-modal-overlay";let g=document.createElement("div");g.className="modal-dialog",g.style.maxWidth="420px";let E=u.length>0?u.map((v,M)=>b(v,M===0)).join(""):`<div style="padding: 20px; text-align: center; color: #666;">No colors available that pass contrast requirements (>= ${s}:1)</div>`;g.innerHTML=`
      <div class="modal-header">
        <button class="modal-close" aria-label="Close">\xD7</button>
      ...
      <div class="modal-footer">
        ...
        <button class="modal-btn modal-btn-create" id="contrast-fix-confirm-apply-btn" style="background: #22c55e; border-color: #22c55e;" ${u.length===0?"disabled":""}>Apply</button>
      </div>
    `,m.appendChild(g),document.body.appendChild(m);let T=g.querySelector("#contrast-fix-confirm-cancel-btn"),A=g.querySelector("#contrast-fix-confirm-apply-btn"),N=g.querySelector("#contrast-fix-ignore-btn"),j=g.querySelector(".modal-close"),z=g.querySelector("#contrast-color-options-container"),D=y,ye=v=>{d=v,z.querySelectorAll(".contrast-color-option-item").forEach(q=>{let he=q.getAttribute("data-color")===v;q.style.border=he?"2px solid #0071e3":"2px solid #e0e0e0",q.style.background=he?"#e3f2fd":"white";let Ee=q.querySelector('input[type="radio"]');Ee&&(Ee.checked=he)})};(()=>{z.querySelectorAll(".contrast-color-option-item").forEach(M=>{M.onclick=q=>{q.preventDefault();let se=M.getAttribute("data-color");ye(se)}})})();let P=()=>{m.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{m.parentNode&&m.remove(),l&&(l.style.display="block")},200)};T.onclick=()=>{D&&D.onCancel?(P(),window.pendingContrastFixAllCallbacks=null,D.onCancel()):P()},j.onclick=()=>{D&&D.onCancel?(P(),window.pendingContrastFixAllCallbacks=null,D.onCancel()):P()},m.onclick=v=>{v.target===m&&(D&&D.onCancel?(P(),window.pendingContrastFixAllCallbacks=null,D.onCancel()):P())},N&&(N.onclick=()=>{P(),D&&D.onIgnore&&(window.pendingContrastFixAllCallbacks=null,D.onIgnore())}),A.onclick=()=>{u.length!==0&&(P(),re(e.id,"\u23F3 Fixing contrast...",!0),parent.postMessage({pluginMessage:{type:"fix-contrast-issue",issue:e,color:d}},"*"),D&&D.onApply&&(window.pendingContrastFixAllCallbacks=null,D.onApply()))}}function ko(e){try{if(ue[e.id]===!0){if(delete ue[e.id],k&&k.issues){let o=k.issues.find(l=>l.id===e.id);if(o){o.ignored=!1;let l=o.originalSeverity||(o.severity==="info"?"error":o.severity);o.severity=l,o.originalSeverity=void 0,e.severity=l,e.ignored=!1}}ve(),wo(e,!1),at(),parent.postMessage({pluginMessage:{type:"notify",message:"\u2705 Issue un-ignored"}},"*")}else{if(!confirm(`Ignore this contrast issue?

Node: ${e.nodeName||"Unnamed"}

This issue will be marked as "Pass with ignore custom" and won't be counted as an error.`))return;if(k&&k.issues){let o=k.issues.find(l=>l.id===e.id);o&&(o.originalSeverity||(o.originalSeverity=o.severity),o.ignored=!0,o.severity="info",e.ignored=!0,e.originalSeverity=o.originalSeverity,e.severity="info")}ue[e.id]=!0,ve(),wo(e,!0),at(),parent.postMessage({pluginMessage:{type:"notify",message:"\u2705 Issue ignored"}},"*")}}catch(t){console.error("Error in handleIgnoreIssue:",t),parent.postMessage({pluginMessage:{type:"notify",message:`\u274C Error: ${t.message}`}},"*")}}function wo(e,t){let o=document.querySelector(`.issue[data-issue-id="${e.id}"]`);if(o)if(t){let l=e.originalSeverity||"error";o.className=o.className.replace(/\b(error|warn)\b/g,"info");let n=o.querySelector(".issue-type");if(n){let a=n.querySelector(".issue-number");if(a){let i=a.textContent;n.innerHTML=`<span class="issue-number">${i}</span> \u2139\uFE0F INFO`}else n.innerHTML=n.innerHTML.replace(/❌|⚠️/g,"\u2139\uFE0F").replace(/ERROR|WARNING/g,"INFO")}if(!o.querySelector(".issue-ignored-tag")){let a=o.querySelector(".issue-body"),i=o.querySelector(".issue-node"),c=document.createElement("div");if(c.className="issue-ignored-tag",c.style.cssText="margin-top: 4px; padding: 4px 8px; background: #e3f2fd; color: #22c55e; border-radius: 4px; font-size: 11px; font-weight: 600; display: inline-block;",c.textContent="\u2713 Pass with ignore custom",i)i.parentNode.insertBefore(c,i.nextSibling);else if(a)a.parentNode.insertBefore(c,a.nextSibling);else{let u=o.querySelector(".issue-header");u?u.parentNode.insertBefore(c,u.nextSibling):o.appendChild(c)}}let s=document.querySelector(`button.btn-ignore[data-id="${e.id}"]`);s&&(s.removeAttribute("disabled"),s.innerHTML="Ignored",s.style.cssText="padding: 6px 12px; border: 1px solid #22c55e; background: #22c55e; border-radius: 6px; font-size: 12px; font-weight: 500; cursor: pointer; transition: all 0.2s; color: white;")}else{let l=e.originalSeverity||(e.severity==="info"?"error":e.severity);o.className=o.className.replace(/\binfo\b/g,l);let n=o.querySelector(".issue-type");if(n){let i=l==="error"?"\u274C":l==="warn"?"\u26A0\uFE0F":"\u2139\uFE0F",c=l.toUpperCase(),u=n.querySelector(".issue-number");if(u){let d=u.textContent;n.innerHTML=`<span class="issue-number">${d}</span> ${i} ${c}`}else n.innerHTML=n.innerHTML.replace(/ℹ️/g,i).replace(/INFO/g,c)}let s=o.querySelector(".issue-ignored-tag");s&&s.remove();let a=document.querySelector(`button.btn-ignore[data-id="${e.id}"]`);a&&(a.removeAttribute("disabled"),a.innerHTML="Ignore",a.style.cssText="padding: 6px 12px; border: 1px solid #64748b; background: #64748b; border-radius: 6px; font-size: 12px; font-weight: 500; cursor: pointer; transition: all 0.2s; color: white;")}}function at(){if(!k||!k.issues)return;let e=k.issues,t={error:e.filter(o=>o.severity==="error"&&!o.ignored).length,warn:e.filter(o=>o.severity==="warn"&&!o.ignored).length,total:e.length};At(e),document.querySelectorAll(".issue-group").forEach(o=>{let l=o.querySelector(".badge");if(l){let n=o.getAttribute("data-issue-type");if(n){let a=e.filter(i=>i.type===n).filter(i=>i.ignored?!1:i.severity==="error"||i.severity==="warn").length;l.textContent=a}}})}function Pn(e){let t=e.message||"",o=t.match(/Padding\s+(\w+)\s+\((\d+)px\)/),l=null,n=null;if(o?(l=o[1],n=parseInt(o[2])):(o=t.match(/Gap\s+\(itemSpacing:\s+(\d+)px\)/),o&&(l="itemSpacing",n=parseInt(o[1]))),!o||!l||n===null){console.error("Cannot parse spacing issue message:",t),alert("Cannot determine spacing property from issue message. Message: "+t);return}let s=document.getElementById("spacing-scale");if(!s||!s.value.trim()){alert("No spacing scale defined. Please add spacing values to the Spacing input.");return}let a=s.value.split(",").map(i=>parseInt(i.trim(),10)).filter(i=>!isNaN(i)&&i>=0).sort((i,c)=>i-c);if(a.length===0){alert("No valid spacing values found in Spacing input.");return}pn(e,l,n,a)}function Rn(e){let o=(e.message||"").match(/Color (#[0-9A-Fa-f]{6})/),l=o?o[1].toUpperCase():null;if(!l){alert("Cannot determine current color from issue message");return}let n=document.getElementById("color-scale");if(!n||!n.value.trim()){alert("No color scale defined. Please add colors to the Color input.");return}let s=n.value.split(",").map(a=>a.trim().toUpperCase()).filter(a=>a&&a.startsWith("#"));if(s.length===0){alert("No valid colors found in Color input.");return}un(e,l,s,J)}function Dn(e){var t;if(e.type==="typography-check"&&e.bestMatch){dn(e,Q);return}if(re(e.id,"\u23F3 Fixing...",!0),!e.bestMatch&&e.type!=="typography-check"){let o=prompt(`Cannot auto-fix this issue.
...
Please provide fix instructions or press Cancel.`);if(o)parent.postMessage({pluginMessage:{type:"fix-issue",issue:e,manualFix:o}},"*");else{let l=document.querySelector(`.issue[data-issue-id="${e.id}"]`)||((t=document.querySelector(`button.btn-fix[data-id="${e.id}"]`))==null?void 0:t.closest(".issue"));if(l){let n=l.querySelector(".fix-message");n&&n.remove()}}return}parent.postMessage({pluginMessage:{type:"fix-issue",issue:e}},"*")}function Co(e,t){let o=document.createElement("div");o.className="modal-overlay",o.id="create-style-modal-overlay";let l=document.createElement("div");l.className="modal-dialog";let n=e.nodeName||"Unnamed";if(e.message&&e.message.includes("(")&&e.message.includes("nodes")){let d=e.message.match(/\((\d+) nodes\)/);d&&(n=`${n} (${d[1]} nodes)`)}l.innerHTML=`
    ...
    `,o.appendChild(l),document.body.appendChild(o);let s=l.querySelector("#style-name-input"),a=l.querySelector("#modal-cancel-btn"),i=l.querySelector("#modal-create-btn"),c=l.querySelector(".modal-close");setTimeout(()=>{s.focus(),s.select()},100),s.onkeydown=d=>{d.key==="Enter"?(d.preventDefault(),i.click()):d.key==="Escape"&&(d.preventDefault(),a.click())};let u=()=>{o.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{o.parentNode&&o.remove()},200)};a.onclick=u,c.onclick=u,o.onclick=d=>{d.target===o&&u()},i.onclick=()=>{let d=s.value.trim();if(!d){s.focus(),s.style.borderColor="#ff3b30",setTimeout(()=>{s.style.borderColor="#0071e3"},2e3);return}u(),t&&t(d)}}function Io(e,t,o={}){if(console.log("[showSuggestApplyModal] Called with issue:",e,"styleName:",t,"options:",o),!e){console.error("[showSuggestApplyModal] Missing issue:",{issue:e,styleName:t});return}let{onApply:l,onIgnore:n,onCancel:s,showIgnore:a=!1,progress:i}=o,c=e.nodeProps||{},u=c.fontFamily||"Unknown",d=c.fontSize!==null&&c.fontSize!==void 0?c.fontSize:null,b=d!==null?`${d}px`:"Unknown",y=c.fontWeight||"Unknown",f=c.lineHeight||"Unknown",m=c.letterSpacing!==null&&c.letterSpacing!==void 0?c.letterSpacing:"Unknown",g=F=>F==null||F==="Unknown"?"":String(F).toLowerCase().trim(),E=F=>{let ce=`typo_${g(u)}_${d}_${g(y)}_${g(f)}_${g(m)}_${F.id}`;return fe(ce,()=>{let K=0;if(g(u)===g(F.fontFamily)&&(K+=25),d!==null&&F.fontSize){let r=Math.abs(d-F.fontSize);r===0?K+=30:r<=2?K+=25:r<=4?K+=20:r<=8&&(K+=10)}return g(y)===g(F.fontWeight)&&(K+=20),g(f)===g(F.lineHeight)&&(K+=15),g(m)===g(F.letterSpacing||"0")&&(K+=10),K})},T=[...Q].map(F=>ot(Ke({},F),{similarity:E(F)})).sort((F,ce)=>t&&F.name===t?-1:t&&ce.name===t?1:ce.similarity-F.similarity).slice(0,5);if(T.length===0){console.error("[showSuggestApplyModal] No typography styles available"),alert("No typography styles available");return}let A=T[0].id,N=(F,ce)=>g(F)!==g(ce),j=(F,ce,K)=>{let r=N(u,F.fontFamily),xe=N(b,`${F.fontSize}px`),ge=N(y,F.fontWeight),Qe=N(f,F.lineHeight),de=N(m,F.letterSpacing||"0%"),et="color: #155724;",De="color: #721c24; background: #f8d7da; padding: 2px 6px; border-radius: 4px; font-weight: 600;",tt=K===0;return`
        ...
        ">
          <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 8px;">
            <div style="display: flex; align-items: center; gap: 10px; flex: 1;">
              ...
              <div>
                <div style="display: flex; align-items: center; gap: 8px;">
                  <span style="font-weight: 600; font-size: 12px; color: #333;">${p(F.name)}</span>
                  ${tt?'<span style="color: #f5a623;">\u2B50</span>':""}
                </div>
                <div style="font-size: 12px; color: #666; margin-top: 2px;">
                  ${p(F.fontFamily)} ${F.fontSize}px ${p(F.fontWeight)}
                </div>
              </div>
            </div>
            <div style="text-align: right;">
      ...
      <div class="modal-body">
        <div style="margin-bottom: 12px; padding: 12px; background: #fff8e6; border-radius: 8px; border-left: 4px solid #f5a623;">
          ...
          <div style="font-size: 12px; color: #333; line-height: 1.6;">
            <div><strong>Font:</strong> ${p(u)} \u2022 ${p(b)} \u2022 ${p(y)}</div>
            <div><strong>Line Height:</strong> ${p(f)} \u2022 <strong>Letter Spacing:</strong> ${p(m)}</div>
          </div>
        </div>
    ...
    `,z.appendChild(D),document.body.appendChild(z);let P=D.querySelector("#suggest-modal-cancel-btn"),v=D.querySelector("#suggest-modal-apply-btn"),M=D.querySelector("#suggest-modal-ignore-btn"),q=D.querySelector(".modal-close"),se=D.querySelector("#style-options-container"),he=F=>{A=F,se.querySelectorAll(".style-option-item").forEach(K=>{let xe=K.getAttribute("data-style-id")===String(F);K.style.border=xe?"2px solid #0071e3":"2px solid #e0e0e0",K.style.borderLeft=xe?"4px solid #0071e3":"4px solid #e0e0e0",K.style.background=xe?"#f8fbff":"white";let ge=K.querySelector('input[type="radio"]');ge&&(ge.checked=xe)})};(()=>{se.querySelectorAll(".style-option-item").forEach(ce=>{ce.onclick=K=>{K.preventDefault();let r=ce.getAttribute("data-style-id");he(r)}})})();let be=()=>{z.style.animation="fadeIn 0.2s ease-out reverse",setTimeout(()=>{z.parentNode&&z.remove()},200)};P.onclick=()=>{be(),a&&o.onCancel&&typeof o.onCancel=="function"&&o.onCancel()},q.onclick=()=>{be(),a&&o.onCancel&&typeof o.onCancel=="function"&&o.onCancel()},z.onclick=F=>{F.target===z&&(be(),a&&o.onCancel&&typeof o.onCancel=="function"&&o.onCancel())},v.onclick=()=>{let F=T.find(ce=>String(ce.id)===String(A));if(!F){console.error("[showSuggestApplyModal] Selected style not found:",A);return}be(),re(e.id,"\u23F3 Applying style...",!0),F.styleId?parent.postMessage({pluginMessage:{type:"apply-figma-text-style",issue:e,styleId:F.styleId,styleName:F.name}},"*"):parent.postMessage({pluginMessage:{type:"apply-typography-style",issue:e,style:F}},"*"),l&&typeof l=="function"&&l()},M&&(M.onclick=()=>{be(),n&&typeof n=="function"&&n()})}function Wn(e){parent.postMessage({pluginMessage:{type:"extract-color-variables"}},"*");function t(o){let l=o.data&&o.data.pluginMessage;if(!l||l.type!=="color-variables-extracted")return;window.removeEventListener("message",t);let n=l.colors||[];if(n.length===0){alert("No color variables found in this file.");return}let s=(e.colorHex||"").toUpperCase(),a=[...n].sort((f,m)=>{let g=f.hex.toUpperCase()===s?0:1,E=m.hex.toUpperCase()===s?0:1;return g!==E?g-E:f.name.localeCompare(m.name)}),i=document.createElement("div");i.className="modal-overlay",i.style.cssText="position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.4);z-index:9999;display:flex;align-items:center;justify-content:center;";let c=document.createElement("div");c.style.cssText="background:#fff;border-radius:12px;padding:20px;max-width:400px;width:90%;max-height:70vh;display:flex;flex-direction:column;";let u=document.createElement("h3");u.style.cssText="margin:0 0 4px 0;font-size:16px;",u.textContent="Select Variable";let d=document.createElement("div");d.style.cssText="font-size:12px;color:#666;margin-bottom:12px;",d.textContent=`Color: ${s} on "${e.nodeName}"`;let b=document.createElement("div");b.style.cssText="overflow-y:auto;flex:1;",a.forEach(f=>{let m=f.hex.toUpperCase()===s,g=document.createElement("div");g.style.cssText=`display:flex;align-items:center;gap:10px;padding:8px 10px;border-radius:8px;cursor:pointer;border:2px solid ${m?"#22c55e":"transparent"};margin-bottom:4px;background:${m?"#f0fdf4":"#fafafa"};`,g.innerHTML=`
        ...
        `,g.onclick=()=>{i.remove(),re(e.id,"\u23F3 Binding variable...",!0),parent.postMessage({pluginMessage:{type:"bind-color-variable-by-name",issue:e,variableName:f.name,variableHex:f.hex}},"*")},b.appendChild(g)});let y=document.createElement("button");y.textContent="Cancel",y.style.cssText="margin-top:12px;padding:8px 16px;border:1px solid #ddd;border-radius:8px;background:#fff;cursor:pointer;font-size:13px;",y.onclick=()=>i.remove(),c.appendChild(u),c.appendChild(d),c.appendChild(b),c.appendChild(y),i.appendChild(c),i.onclick=f=>{f.target===i&&i.remove()},document.body.appendChild(i)}window.addEventListener("message",t)}function ft(e){if(!e||!e.nodeProps)return null;for(let t of Q)if(yn(e.nodeProps,t)===100)return console.log("[100% Match] node:",e.id,e.nodeName,"| nodeProps:",JSON.stringify({f:e.nodeProps.fontFamily,s:e.nodeProps.fontSize,w:e.nodeProps.fontWeight,lh:e.nodeProps.lineHeight,ls:e.nodeProps.letterSpacing}),"| style:",t.name,JSON.stringify({f:t.fontFamily,s:t.fontSize,w:t.fontWeight,lh:t.lineHeight,ls:t.letterSpacing})),t;return null}function ml(e){let t=(e||[]).filter(d=>d.type!=="typography-check"||!d.nodeProps?!1:ft(d)!==null);if(t.length===0){alert("No typography issues with 100% match found.");return}let o=0,l=0,n=0,s=document.getElementById("btn-fix-all-100");s&&(s.disabled=!0,s.textContent=`Fixing... (0/${t.length})`);function a(){s&&(s.textContent=`Fixing... (${o}/${t.length})`)}function i(){s&&(s.disabled=!1,s.textContent=`Fix all now (${t.length-l})`,l>=t.length&&(s.style.display="none")),alert(`\u2705 Done!

Processed ${t.length} item(s):
\u2022 Applied: ${l}
\u2022 Failed: ${n}`)}function c(d){let b=d.data&&d.data.pluginMessage;!b||b.type!=="apply-typography-style-result"||(o++,b.success?l++:n++,a(),o>=t.length?(window.removeEventListener("message",c),setTimeout(i,500)):setTimeout(()=>u(t[o]),300))}function u(d){let b=ft(d);if(!b){o++,n++,a(),o>=t.length?(window.removeEventListener("message",c),setTimeout(i,500)):setTimeout(()=>u(t[o]),300);return}re(d.id,"\u23F3 Applying style...",!0),b.styleId?parent.postMessage({pluginMessage:{type:"apply-figma-text-style",issue:d,styleId:b.styleId,styleName:b.name}},"*"):parent.postMessage({pluginMessage:{type:"apply-typography-style",issue:d,style:b}},"*")}window.addEventListener("message",c),u(t[0])}function Un(e){let t=["typography-check","typography-style"],o=(e||[]).filter(v=>t.includes(v.type)&&v.nodeProps&&ft(v)!==null),l=[];o.forEach(function(v){v.subIssues&&v.subIssues.length>1?v.subIssues.forEach(function(M){M.nodeProps&&ft(Object.assign({},v,M))!==null&&l.push(Object.assign({},v,M))}):l.push(v)});let n=(e||[]).some(v=>v.type==="color-variable"&&v.matchingVariable&&(!v.colorOpacity||v.colorOpacity>=1));if(console.log("[Fix All Now] Color variable fixable:",(e||[]).filter(v=>v.type==="color-variable"&&v.matchingVariable&&(!v.colorOpacity||v.colorOpacity>=1)).length,", Typography 100% match:",l.length),l.length===0&&!n){alert("No auto-fixable issues found.");return}let s=document.getElementById("btn-fix-all-100");s&&(s.disabled=!0,s.textContent="Fixing...");let a=0,i=0,c=0,u=0,d=!1,b=l.length+(n?1:0),y=0,f=document.getElementById("fix-all-progress");f&&f.remove();let m=document.createElement("div");m.id="fix-all-progress",m.className="fix-all-progress",m.innerHTML=`
    ...
    `;let g=document.getElementById("results-issues");g&&g.parentNode.insertBefore(m,g);let E=m.querySelector(".fix-all-progress-text"),T=m.querySelector(".fix-all-progress-fill"),A=m.querySelector(".fix-all-progress-cancel");A.onclick=()=>{d=!0,j()};function N(v){y++;let M=Math.round(y/b*100);E&&(E.textContent=v+" "+y+"/"+b),T&&(T.style.width=M+"%")}function j(){s&&(s.disabled=!1,s.textContent="Fix all now"),m&&m.parentNode&&m.remove(),c>0&&k&&k.issues&&(k.issues=k.issues.filter(q=>q.type!=="color-variable"||!q.matchingVariable)),setTimeout(function(){at(),k&&k.issues&&Ve(k.issues,!1)},300);let v=c+a,M=u+i;alert((d?`Cancelled!

`:`Done!

`)+"Color variables: "+c+" applied, "+u+` failed
Typography: `+a+" applied, "+i+` failed

Total: `+(v+M)+" processed")}function z(){s&&(s.textContent="Fixing colors...");function v(M){let q=M.data&&M.data.pluginMessage;!q||q.type!=="batch-bind-color-variables-result"||(window.removeEventListener("message",v),c=q.applied||0,u=q.failed||0,N("Colors done."),l.length>0&&!d?setTimeout(function(){ye()},300):j())}window.addEventListener("message",v),parent.postMessage({pluginMessage:{type:"batch-bind-color-variables",skipOpacity:!0}},"*")}let D=0;function ye(){s&&(s.textContent="Fixing Text Style..."),window.addEventListener("message",x),P(l[0])}function x(v){let M=v.data&&v.data.pluginMessage;!M||M.type!=="apply-typography-style-result"||(D++,M.success?a++:i++,N("Fixing Text Style"),d||D>=l.length?(window.removeEventListener("message",x),j()):setTimeout(function(){P(l[D])},300))}function P(v){let M=ft(v);if(!M){D++,i++,N("Fixing Text Style"),d||D>=l.length?(window.removeEventListener("message",x),j()):setTimeout(function(){P(l[D])},300);return}re(v.id,"\u23F3 Applying style...",!0),M.styleId?parent.postMessage({pluginMessage:{type:"apply-figma-text-style",issue:v,styleId:M.styleId,styleName:M.name}},"*"):parent.postMessage({pluginMessage:{type:"apply-typography-style",issue:v,style:M}},"*")}n?z():l.length>0&&ye()}function jn(e,t){if(!t||t.length===0){alert("No issues to process");return}let o=0,l=0,n=0,s=!1,a=document.getElementById("fix-all-progress");a&&a.remove();let i=document.createElement("div");i.id="fix-all-progress",i.className="fix-all-progress",i.innerHTML=`
      <div class="fix-all-progress-info">
        <span class="fix-all-progress-text">Fixing... 0/${t.length}</span>
        <button class="fix-all-progress-cancel" title="Cancel">\u2715</button>
      </div>
      <div class="fix-all-progress-bar"><div class="fix-all-progress-fill" style="width: 0%"></div></div>
    `;let c=document.getElementById("results-issues");c&&c.parentNode.insertBefore(i,c);let u=i.querySelector(".fix-all-progress-text"),d=i.querySelector(".fix-all-progress-fill"),b=i.querySelector(".fix-all-progress-cancel");b.onclick=()=>{s=!0};function y(){let g=Math.round(o/t.length*100);u&&(u.textContent="Fixing... "+o+"/"+t.length),d&&(d.style.width=g+"%")}function f(){i&&i.parentNode&&i.remove();let E=(s?`Cancelled!

`:`Done!

`)+`Processed ${o} item(s):
\u2022 Applied: ${l}
\u2022 Ignored: ${n}`;alert(E)}function m(){if(s||o>=t.length){f();return}let g=t[o];o++,y(),parent.postMessage({pluginMessage:{type:"select-node",id:g.id}},"*");let E={current:o,total:t.length};if(g.type==="typography-check"||g.type==="typography-style"){let T=g.bestMatch&&g.bestMatch.name&&typeof g.bestMatch.name=="string"&&g.bestMatch.name.trim().length>0?g.bestMatch.name:null;if(!T){n++,m();return}Io(g,T,{showIgnore:!0,progress:E,onApply:()=>{l++,setTimeout(()=>{m()},500)},onIgnore:()=>{n++,m()},onCancel:()=>{s=!0,f()}})}else if(g.type==="color"){let T=St(g),N=(g.message||"").match(/Color (#[0-9A-Fa-f]{6})/),j=N?N[1].toUpperCase():null;On(g,j,T,{progress:E,onApply:()=>{l++,setTimeout(()=>{m()},500)},onIgnore:()=>{n++,m()},onCancel:()=>{s=!0,f()}})}else if(g.type==="spacing"){let T=kt(g),N=(g.message||"").match(/Padding\s+(\w+)\s+\((\d+)px\)/);if(N){let j=N[1],z=parseInt(N[2]);Vn(g,j,z,T,{progress:E,onApply:()=>{l++,setTimeout(()=>{m()},500)},onIgnore:()=>{n++,m()},onCancel:()=>{s=!0,f()}})}else n++,m()}else if(g.type==="autolayout")wn(g,{progress:E,onApply:()=>{l++,setTimeout(()=>{m()},500)},onIgnore:()=>{n++,m()},onCancel:()=>{s=!0,f()}});else if(g.type==="position")Mn(g,{progress:E,onApply:()=>{l++,setTimeout(()=>{m()},500)},onIgnore:()=>{n++,m()},onCancel:()=>{s=!0,f()}});else if(g.type==="group")$n(g,{progress:E,onApply:()=>{l++,setTimeout(()=>{m()},500)},onIgnore:()=>{n++,m()},onCancel:()=>{s=!0,f()}});else if(g.type==="empty-frame")Tn(g,{progress:E,onApply:()=>{l++,setTimeout(()=>{m()},500)},onIgnore:()=>{n++,m()},onCancel:()=>{s=!0,f()}});else if(g.type==="text-size-mobile")window.pendingTextSizeFixAllCallbacks={progress:E,onApply:()=>{l++,setTimeout(()=>{m()},500)},onIgnore:()=>{n++,m()},onCancel:()=>{s=!0,f()}},jt(g);else if(g.type==="contrast")window.pendingContrastFixAllCallbacks={progress:E,onApply:()=>{l++,setTimeout(()=>{m()},500)},onIgnore:()=>{n++,m()},onCancel:()=>{s=!0,f()}},Ot(g);else if(g.type==="color-variable")if(g.matchingVariable&&g.matchingVariable.id){let T=function(A){let N=A.data&&A.data.pluginMessage;!N||N.type!=="bind-color-variable-result"||N.issueId===g.id&&(window.removeEventListener("message",T),N.success?l++:n++,setTimeout(()=>{m()},300))};re(g.id,"\u23F3 Binding variable...",!0),window.addEventListener("message",T),parent.postMessage({pluginMessage:{type:"bind-color-variable",issue:g,variableId:g.matchingVariable.id}},"*")}else n++,m();else n++,m()}m()}function On(e,t,o,l={}){let n=Object.keys(J);Dt(e,t,o,J,n,ot(Ke({},l),{showIgnore:!0}))}function Vn(e,t,o,l,n={}){let s=document.getElementById("spacing-scale"),a=[0,4,8,12,16,24,32,40,48,64,72,80,88,96];if(s&&s.value.trim()){let i=s.value.split(",").map(c=>parseInt(c.trim(),10)).filter(c=>!isNaN(c));i.length>0&&(a=i)}Rt(e,t,o,l,a,ot(Ke({},n),{showIgnore:!0}))}function $o(e,t){if(!e||!t){console.error("handleApplyFigmaTextStyle: missing issue or style");return}re(e.id,"\u23F3 Applying style...",!0),parent.postMessage({pluginMessage:{type:"apply-figma-text-style",issue:e,styleId:t.id,styleName:t.name}},"*")}function Vt(e,t){if(!e||!t){console.error("handleApplyTypographyStyle: missing issue or styleName");return}Io(e,t)}function Eo(e){if(console.log("handleCreateTextStyle called",e),!e){console.error("handleCreateTextStyle: issue is null/undefined");return}Co(e,t=>{console.log("handleCreateTextStyle: sending message",{type:"create-text-style",issueId:e.id,styleName:t}),re(e.id,"\u23F3 Creating style...",!0),parent.postMessage({pluginMessage:{type:"create-text-style",issue:e,styleName:t}},"*")})}function fl(e,t){if(e==="typography-style"){let n={nodeName:`${t.length} text node(s)`,message:`Found ${t.length} text node(s) without text style`};Co(n,s=>{t.forEach(a=>{re(a.id,"\u23F3 Creating style...",!0)}),parent.postMessage({pluginMessage:{type:"create-text-style-all",issues:t,styleName:s}},"*")});return}let o=t.filter(n=>n.bestMatch&&n.type==="typography-check"),l=t.filter(n=>!n.bestMatch||n.type!=="typography-check");if(o.length===0){alert(`No auto-fixable issues found in ${Tt(e)}.
...
The ${l.length} issues will need to be fixed manually.`)||parent.postMessage({pluginMessage:{type:"fix-all-issues",issues:o,issueType:e}},"*")}function Gn(e){console.log("filterAndSearchIssues called",{totalIssues:e.length,currentFilter:Ae,currentSearch:Ie});let t=e;if(Ae!=="all"&&(t=t.filter(o=>o.severity===Ae),console.log("After severity filter:",t.length)),Ie.trim()){let o=Ie.toLowerCase();t=t.filter(l=>{let n=(l.message||"").toLowerCase(),s=(l.nodeName||"").toLowerCase(),a=(l.type||"").toLowerCase();return n.includes(o)||s.includes(o)||a.includes(o)}),console.log("After search filter:",t.length)}return console.log("Final filtered issues:",t.length),t}function Ve(e=[],t=!1,o={}){let{skipSave:l=!1,restoreTimestamp:n=null,skipTabSwitch:s=!1}=o;s||Le("issues"),document.getElementById("issues-count").textContent=e.length,e&&Array.isArray(e)&&e.forEach(x=>{ue[x.id]===!0&&(x.ignored=!0,x.originalSeverity||(x.originalSeverity=x.severity),x.severity="info")});let a=k.issues!==e;k.issues=e;let i=n||new Date().toISOString();k.timestamp=i,Ct=!1,(t||a)&&(console.log("Resetting filters for new data"),Ae="all",Oe="all",We&&(We.value=""),Ie="",Be&&(Be.style.display="none"),$e&&$e.length>0&&$e.forEach(x=>{x.classList.remove("active"),x.getAttribute("data-filter")==="all"&&x.classList.add("active")}));let c=document.getElementById("filter-controls"),u=document.getElementById("color-type-filter"),d=document.getElementById("filter-buttons");c&&(c.style.display=e.length>0?"flex":"none"),u&&(u.style.display="none"),d&&(d.style.display="flex");let b=document.getElementById("export-group");b.style.display=e.length>0?"flex":"none",console.log("About to filter with:",{currentFilter:Ae,currentSearch:Ie});let y=Gn(e),f=new Set;if(a||(document.querySelectorAll(".issue-group").forEach(P=>{let v=P.getAttribute("data-issue-type");v&&!P.classList.contains("collapsed")&&f.add(v)}),console.log("Saved expanded groups:",Array.from(f))),zt("issues"),y.length===0&&e.length===0){ne.innerHTML=`
              <div class="empty-state success">
                <div class="icon">\u2705</div>
                <p><strong>No issues found!</strong></p>
                <p style="margin-top: 8px; font-size: 12px;">Your design passed all configured checks.</p>
              </div>
            `;return}if(y.length===0&&e.length>0&&Ae!=="all"){ne.innerHTML=`
              <div class="empty-state">
                <div class="icon">\u{1F50D}</div>
                <p><strong>No results found</strong></p>
                <p style="margin-top: 8px; font-size: 12px;">Try changing the filter or search keyword.</p>
              </div>
            `;return}let m={error:y.filter(x=>x.severity==="error"&&!x.ignored).length,warn:y.filter(x=>x.severity==="warn"&&!x.ignored).length,total:y.length,originalTotal:e.length},g=0,E=0;y.forEach(function(x){x.ignored||((x.type==="typography-check"||x.type==="typography-style")&&x.nodeProps&&ft(x)!==null&&g++,x.type==="color-variable"&&(console.log("[Header Count] color-variable:",x.id,"matchingVariable:",!!x.matchingVariable,"opacity:",x.colorOpacity),x.matchingVariable&&(!x.colorOpacity||x.colorOpacity>=1)&&E++))});let T=g+E;console.log("[Header Count] typo100:",g,"colorVar:",E,"total:",T);let A=document.createElement("div");A.className="results-header";let N=T>0;A.innerHTML=`<h3>Check Result</h3><button class="btn-fix-all" id="btn-fix-all-100"${N?"":' style="display:none"'}>Fix all now (${T})</button>`,ne.appendChild(A);let j=A.querySelector("#btn-fix-all-100");j&&(j.onclick=x=>{x.preventDefault(),x.stopPropagation(),Un(e)}),At(e);let z=y.reduce((x,P)=>(x[P.type]=x[P.type]||[],x[P.type].push(P),x),{}),D=e.reduce((x,P)=>(x[P.type]=x[P.type]||[],x[P.type].push(P),x),{}),ye=["naming","autolayout","spacing","color","color-variable","typography","typography-style","typography-check","line-height","position","duplicate","group","component","empty-frame","nested-group","contrast","text-size-mobile"];for(let x of ye){let P=z[x]||[],v=P.length,M=P.filter(r=>r.ignored?!1:r.severity==="error"||r.severity==="warn").length;if(v===0&&e.length===0||v===0&&Ae!=="all")continue;let q=document.createElement("div"),se=f.has(x);q.className=se?"issue-group":"issue-group collapsed",q.setAttribute("data-issue-type",x);let he=document.createElement("div");he.className="issue-group-header";let Ee=(D[x]||[]).some(r=>{try{if(!r)return!1;switch(r.type){case"color":return St(r)!==null;case"spacing":return kt(r)!==null;case"autolayout":return typeof pt=="function"&&pt(r)!==null;case"text-size-mobile":return typeof it=="function"&&it(r)!==null;case"contrast":return typeof mt=="function"&&mt(r)!==null;case"typography-style":case"typography-check":return r.bestMatch&&r.bestMatch.name&&typeof r.bestMatch.name=="string"&&r.bestMatch.name.trim().length>0&&Mt(r);case"position":return typeof st=="function"&&st(r)!==null;case"duplicate":case"component":return typeof ut=="function"&&ut(r)!==null;case"group":return!0;case"empty-frame":return typeof gt=="function"&&gt(r)!==null;default:return!1}}catch(xe){return console.error("[hasSuggestFixButton] Error checking issue:",r,xe),!1}});he.innerHTML=`
              ...
              ${v>0&&x!=="typography"&&x!=="line-height"&&x!=="naming"&&x!=="component"&&x!=="duplicate"&&Ee?`<button class="btn-fix-all" data-type="${x}">Fix all now</button>`:""}
            `;let be=he.querySelector(".issue-group-toggle"),F=()=>{q.classList.contains("collapsed")?(q.classList.remove("collapsed"),K.style.display="block",setTimeout(()=>{K.style.opacity="1"},10)):(K.style.opacity="0",setTimeout(()=>{q.classList.add("collapsed"),K.style.display="none"},200))};be.onclick=r=>{r.stopPropagation(),F()},he.onclick=r=>{r.target!==be&&!be.contains(r.target)&&F()};let ce=he.querySelector(".btn-fix-all");ce&&(ce.onclick=r=>{try{r.preventDefault(),r.stopPropagation();let xe=(D[x]||[]).filter(ge=>{if(!ge)return!1;switch(ge.type){case"color":return St(ge)!==null;case"spacing":return kt(ge)!==null;case"autolayout":return typeof pt=="function"&&pt(ge)!==null;case"text-size-mobile":return typeof it=="function"&&it(ge)!==null;case"contrast":return typeof mt=="function"&&mt(ge)!==null;case"typography-style":case"typography-check":return ge.bestMatch&&ge.bestMatch.name&&typeof ge.bestMatch.name=="string"&&ge.bestMatch.name.trim().length>0&&Mt(ge);case"position":return typeof st=="function"&&st(ge)!==null;case"duplicate":case"component":return typeof ut=="function"&&ut(ge)!==null;case"group":return!0;case"empty-frame":return typeof gt=="function"&&gt(ge)!==null;default:return!1}});if(xe.length===0){alert("No issues with suggest fix available");return}console.log("[Fix All]",x,xe.length,"fixable issues"),jn(x,xe)}catch(xe){console.error("[Fix All] Error:",xe.message),alert("Error: "+xe.message)}}),q.appendChild(he);let K=document.createElement("div");if(K.className="issue-group-content",se?(K.style.display="block",K.style.opacity="1"):K.style.display="none",v===0){let r=document.createElement("div");r.className="issue info",r.style.opacity="0.7",r.innerHTML=`
                <div class="issue-header">
                  <div>
              ...
              `,K.appendChild(r)}else z[x].forEach((r,xe)=>{let ge=xe+1,Qe=ue[r.id]===!0;if(Qe&&(r.originalSeverity||(r.originalSeverity=r.severity),r.severity="info",r.ignored=!0),r.type==="typography-check"){let W=document.createElement("div");K.appendChild(W);let ie=xn(r);if(ie){let Ue=document.createElement("span");Ue.className="issue-number",Ue.textContent=`#${ge}`,Ue.style.cssText="position: absolute; left: 8px; top: 8px; font-weight: bold; opacity: 0.5; font-size: 11px;",ie.style.position="relative",ie.style.paddingLeft="40px",ie.insertBefore(Ue,ie.firstChild),K.replaceChild(ie,W)}return}let de=document.createElement("div"),et=Qe?"info":r.severity;de.className=`issue ${et}`,de.setAttribute("data-issue-id",r.id);let De=p(r.message);if(r.type==="contrast"){let W=[];if(r.textColor&&W.push(`Text color: <code style="background: ${p(r.textColor)}; padding: 2px 6px; border-radius: 3px; color: ${wt(r.textColor)};">${p(r.textColor)}</code> (${r.textColorNode||r.nodeName||"Unnamed"})`),r.backgroundColor){let ie="Background:",Ue="",xt="";r.isGradient&&r.gradientString?(ie="Background (gradient):",Ue=`<code style="background: ${p(r.backgroundColor)}; padding: 2px 6px; border-radius: 3px; color: ${wt(r.backgroundColor)}; font-family: 'SF Mono', Monaco, monospace; font-size: 11px;">${p(r.gradientString)}</code>`,xt=" <span style='font-size: 11px; color: #999;'>(average: "+p(r.backgroundColor)+")</span>"):Ue=`<code style="background: ${p(r.backgroundColor)}; padding: 2px 6px; border-radius: 3px; color: ${wt(r.backgroundColor)};">${p(r.backgroundColor)}</code>`,r.fromSibling&&(xt+=" <span style='font-size: 11px; color: #3b82f6;'>(from sibling layer)</span>"),W.push(`${ie} ${Ue}${xt} (${r.backgroundColorNode||"Unknown"})`)}W.length>0&&(De+=`<div style="margin-top: 8px; font-size: 10px; color: #666;">${W.join(" | ")}</div>`)}de.innerHTML=`
                <div class="issue-header">
                  <div>
          ...
          `,me.appendChild(A);for(let[j,z]of Object.entries(T)){let D=document.createElement("div");D.className="issue-group collapsed";let ye=document.createElement("div");ye.className="issue-group-header",ye.innerHTML=`
            ...
            `;let x=document.createElement("div");x.className="issue-group-content",x.style.display="none";let P=ye.querySelector(".issue-group-toggle"),v=()=>{D.classList.contains("collapsed")?(D.classList.remove("collapsed"),x.style.display="block",setTimeout(()=>{x.style.opacity="1"},10)):(x.style.opacity="0",setTimeout(()=>{D.classList.add("collapsed"),x.style.display="none"},200))};if(P.onclick=q=>{q.stopPropagation(),v()},ye.onclick=q=>{q.target!==P&&!P.contains(q.target)&&v()},D.appendChild(ye),Array.isArray(z.values)&&z.values.length>0){let q=document.createElement("div");q.className="token-list",z.values.forEach((se,he)=>{let Ee=he+1,be=document.createElement("div");be.className="token-item";let F=se.value,ce=se.nodes||[],K=ce.length>0?ce[0]:null,r=typeof se.totalNodes=="number"?se.totalNodes:ce.length,xe=se.colorType||null,ge="";if(j==="colors"){let ke=xe?`<span class="token-color-type">${p(xe)}</span>`:"";ge=`
                  <span class="token-number">#${Ee}</span>
                  <span class="token-color-preview" style="background-color: ${p(F)}"></span>
                  <code>${p(F)}</code>
                  ${ke}
                `}else if(j==="gradients"){let ke=xe?`<span class="token-color-type">${p(xe)}</span>`:"";ge=`
                  <span class="token-number">#${Ee}</span>
                  <span class="token-gradient-preview" style="background: ${p(F)}"></span>
                  <code>${p(F)}</code>
                  ${ke}
                `}else ge=`<span class="token-number">#${Ee}</span><code>${p(String(F))}</code>`;let Qe=se._matchedBy||[],de=se._matchedNodeNames||[],et=Qe.includes("nodeName"),De="";K&&K.name&&(et&&de.length>0?De=`<div class="token-node-name token-matched-by-name">
                    <span class="match-indicator">\u{1F50D} Matched in:</span> ${de.map(Xe=>`<span class="token-matched-node">${p(Xe)}</span>`).join(", ")}
                  </div>`:De=`<div class="token-node-name">Node: ${p(K.name)}</div>`);let tt="";if(j==="fontWeight"){let Xe=Array.isArray(se.fontFamilies)?se.fontFamilies:null;if(!Xe){let rt={};ce.forEach(Fe=>{let Ze=Fe&&Fe.fontFamily?String(Fe.fontFamily):"Unknown";rt[Ze]=(rt[Ze]||0)+1}),Xe=Object.entries(rt).map(([Fe,Ze])=>({family:Fe,count:Ze})).sort((Fe,Ze)=>Ze.count-Fe.count||Fe.family.localeCompare(Ze.family))}Array.isArray(Xe)&&Xe.length>0&&(tt=`
                    <div class="token-note">
                      <div class="token-note-label">Font-family:</div>
                      <ul class="token-note-list">${Xe.map(Fe=>`<li><code>${p(Fe.family)}</code> (${Fe.count})</li>`).join("")}</ul>
                    </div>
                  `)}be.innerHTML=`
                ...
                ${De}
                ${tt}
              `;let Ye=be.querySelector("button.btn-select");Ye&&(Ye.onclick=()=>{document.querySelectorAll(".btn-select.active").forEach(ke=>ke.classList.remove("active")),document.querySelectorAll(".issue.selected, .token-item.selected").forEach(ke=>ke.classList.remove("selected")),Ye.classList.add("active"),be.classList.add("selected"),parent.postMessage({pluginMessage:{type:"select-node",id:K.id}},"*")}),q.appendChild(be)}),x.appendChild(q)}else{let q=document.createElement("div");q.className="token-empty-message",q.textContent="No tokens in this group.",x.appendChild(q)}D.appendChild(x),me.appendChild(D)}l||Wt({issues:k.issues,issuesTimestamp:k.timestamp,tokens:e,tokensTimestamp:c,lastActiveTab:"tokens",scanMode:k.scanMode||null,context:k.context||null})}function lt(e){let t=document.getElementById("validation-error"),o=document.getElementById("validation-error-message"),l=document.getElementById("btn-close-validation-error");t&&o&&(o.textContent=e,t.style.display="block",S.style.display="block",C.style.display="none",h.style.display="none",S.disabled=!1,I.disabled=!1,setTimeout(()=>{t.style.display==="block"&&(t.style.display="none")},1e4),l&&(l.onclick=()=>{t.style.display="none"}))}function Mo(e){var j,z,D,ye,x,P,v;S.style.display="none",C.style.display="block",h.style.display="block",w.style.transition="none",w.style.width="0%",$.textContent="0%",setTimeout(()=>{w.style.transition="width 0.3s"},10),X();let t=document.getElementById("spacing-scale"),o=document.getElementById("spacing-threshold"),l=document.getElementById("color-scale"),n=document.getElementById("font-size-scale"),s=document.getElementById("font-size-threshold"),a=document.getElementById("line-height-scale"),i=document.getElementById("line-height-threshold"),c=document.getElementById("line-height-baseline-threshold"),u=t?t.value.trim():"",d=o?parseInt(o.value,10):100,b=l?l.value.trim():"",y=n?n.value.trim():"",f=s?parseInt(s.value,10):100,m=a?a.value.trim():"",g=i?parseInt(i.value,10):300,E=c?parseInt(c.value,10):120,T={checkStyle:((j=document.getElementById("rule-typo-style"))==null?void 0:j.checked)||!1,checkFontFamily:((z=document.getElementById("rule-font-family"))==null?void 0:z.checked)||!1,checkFontSize:((D=document.getElementById("rule-font-size"))==null?void 0:D.checked)||!1,checkFontWeight:((ye=document.getElementById("rule-font-weight"))==null?void 0:ye.checked)||!1,checkLineHeight:((x=document.getElementById("rule-line-height"))==null?void 0:x.checked)||!1,checkLetterSpacing:((P=document.getElementById("rule-letter-spacing"))==null?void 0:P.checked)||!1,checkWordSpacing:((v=document.getElementById("rule-word-spacing"))==null?void 0:v.checked)||!1},A=document.getElementById("scan-skip-names"),N=A?A.value.trim():"not check design, sticky note";parent.postMessage({pluginMessage:{type:"scan",mode:e,spacingScale:u,spacingThreshold:d,colorScale:b,fontSizeScale:y,fontSizeThreshold:f,lineHeightScale:m,lineHeightThreshold:g,lineHeightBaselineThreshold:E,typographyStyles:Q,typographyRules:T,ignoredIssues:ue,skipNames:N}},"*"),console.log("Message sent:",{type:"scan",mode:e})}S.onclick=()=>{var e;console.log("btnScan clicked");try{let t=document.getElementById("validation-error");t&&(t.style.display="none");let o=((e=document.querySelector('input[name="scope"]:checked'))==null?void 0:e.value)||"page",l=document.getElementById("spacing-scale"),n=document.getElementById("spacing-threshold"),s=document.getElementById("color-scale"),a=document.getElementById("font-size-scale"),i=document.getElementById("font-size-threshold"),c=document.getElementById("line-height-scale"),u=document.getElementById("line-height-threshold"),d=document.getElementById("line-height-baseline-threshold"),b=l?l.value.trim():"",y=n?parseInt(n.value,10):100,f=s?s.value.trim():"",m=a?a.value.trim():"",g=i?parseInt(i.value,10):100,E=c?c.value.trim():"",T=u?parseInt(u.value,10):300,A=d?parseInt(d.value,10):120;if(b&&!/^\d+(\s*,\s*\d+)*$/.test(b)){lt("Spacing guidelines format is incorrect. Please enter the numbers separated by commas (e.g. 4, 8, 12, 16)");return}if(f){let N=f.split(",").map(D=>D.trim()).filter(D=>D),j=/^#[0-9a-fA-F]{3,8}$/,z=N.filter(D=>!j.test(D));if(z.length>0){lt(`Color format is incorrect. Invalid colors: ${z.join(", ")}. Please use hex format only.`);return}}if(m&&!/^\d+(\s*,\s*\d+)*$/.test(m)){lt("Font-size scale format is incorrect. Please enter the numbers separated by commas (e.g. 32, 24, 20, 18)");return}if(E){let N=E.split(",").map(z=>z.trim()).filter(z=>z);if(!N.every(z=>z.toLowerCase()==="auto"||/^\d+$/.test(z))||N.length===0){lt('Line-height scale format is incorrect. Please enter "auto" and/or numbers separated by commas.');return}}if(isNaN(y)||y<0){lt("Spacing threshold must be a number >= 0");return}if(isNaN(g)||g<0){lt("Font-size threshold must be a number >= 0");return}if(isNaN(T)||T<0){lt("Line-height threshold must be a number >= 0");return}if(isNaN(A)||A<0){lt("Line-height baseline threshold must be a number >= 0");return}ne.innerHTML=`
        <div class="scanning">
          <div class="spinner"></div>
          <p>Checking design size...</p>
        </div>
      `,Le("issues"),S.disabled=!0,I.disabled=!0,k.scanMode=o,ve(),Se={scope:o},parent.postMessage({pluginMessage:{type:"get-node-count",mode:o}},"*")}catch(t){console.error("Error in btnScan.onclick:",t),ne.innerHTML=`<div class="error-message">Error: ${p(t.message)}</div>`,Le("issues"),S.disabled=!1,I.disabled=!1}},I.onclick=()=>{var e;console.log("btnExtractTokens clicked");try{I.style.display="none",C.style.display="block",h.style.display="block",w.style.transition="none",w.style.width="0%",$.textContent="0%",setTimeout(()=>{w.style.transition="width 0.3s"},10);let t=((e=document.querySelector('input[name="scope"]:checked'))==null?void 0:e.value)||"page";me.innerHTML=`
        <div class="scanning">
          <div class="spinner"></div>
          <p>Extracting design tokens... Please wait</p>
        </div>
      `,Le("tokens"),S.disabled=!0,I.disabled=!0,k.scanMode=t,parent.postMessage({pluginMessage:{type:"extract-tokens",mode:t}},"*"),console.log("Message sent:",{type:"extract-tokens",mode:t})}catch(t){console.error("Error in btnExtractTokens.onclick:",t),me.innerHTML=`<div class="error-message">Error: ${p(t.message)}</div>`,Le("tokens"),S.disabled=!1,I.disabled=!1}},R.onclick=()=>{try{Jn(k.tokens)}catch(e){console.error("Failed to fill spacing guidelines from tokens",e)}},_.onclick=()=>{try{let e=k.tokens;if(!e||!Array.isArray(e.colors)||!e.colors.length){alert("No color tokens found. Please run 'Extract Design Tokens' first.");return}let t=document.getElementById("color-scale");if(!t)return;let l=(e.colors||[]).map(s=>String(s&&s.value!==void 0?s.value:"").trim().toUpperCase()).filter(s=>s&&s.startsWith("#"));if(!l.length)return;let n=Array.from(new Set(l)).sort((s,a)=>dt(s)-dt(a));t.value=n.join(", "),typeof He=="function"&&He();try{t.focus(),t.setSelectionRange(t.value.length,t.value.length)}catch(s){}}catch(e){console.error("Failed to fill color from tokens",e)}},O.onclick=()=>{parent.postMessage({pluginMessage:{type:"extract-color-styles"}},"*")};let zo=document.getElementById("btn-extract-color-variables");zo&&(zo.onclick=()=>{parent.postMessage({pluginMessage:{type:"extract-color-variables"}},"*")}),L.onclick=()=>{try{let e=k.tokens;if(!e||!Array.isArray(e.fontSize)||!e.fontSize.length){alert("No font size tokens found. Please run 'Extract Design Tokens' first.");return}let t=document.getElementById("font-size-scale");if(!t)return;let o=document.getElementById("font-size-threshold"),l=o?parseInt(o.value,10):100,n=isNaN(l)?100:l,s=e.fontSize.map(i=>parseInt(String(i&&i.value!==void 0?i.value:"").trim(),10)).filter(i=>!isNaN(i)&&i<=n);if(!s.length)return;let a=Array.from(new Set(s)).sort((i,c)=>c-i);t.value=a.join(", "),t.focus(),t.setSelectionRange(t.value.length,t.value.length)}catch(e){console.error("Failed to fill font size from tokens",e)}},U.onclick=()=>{try{let e=k.tokens;if(!e||!Array.isArray(e.lineHeight)||!e.lineHeight.length){alert("No line height tokens found. Please run 'Extract Design Tokens' first.");return}let t=document.getElementById("line-height-scale");if(!t)return;let o=document.getElementById("line-height-threshold"),l=o?parseInt(o.value,10):300,n=isNaN(l)?300:l,s=[];if(e.lineHeight.forEach(d=>{let b=String(d&&d.value!==void 0?d.value:"").trim();if(b==="auto")s.push("auto");else{let y=parseFloat(b);!isNaN(y)&&y<=n&&s.push(y)}}),!s.length)return;let a=s.includes("auto"),i=s.filter(d=>d!=="auto"),c=Array.from(new Set(i)).sort((d,b)=>d-b),u=a?["auto",...c]:c;t.value=u.join(", "),t.focus(),t.setSelectionRange(t.value.length,t.value.length)}catch(e){console.error("Failed to fill line height from tokens",e)}},V.onclick=()=>{try{if(!Q||!Array.isArray(Q)||Q.length===0){alert("No typography styles defined. Please add typography styles or extract from Figma first.");return}let e=document.getElementById("font-size-scale");if(!e)return;let t=Q.map(l=>{let n=parseInt(String(l.fontSize||"").trim(),10);return isNaN(n)?null:n}).filter(l=>l!==null);if(!t.length){alert("No valid font sizes found in typography styles.");return}let o=Array.from(new Set(t)).sort((l,n)=>n-l);e.value=o.join(", "),ve(),e.focus(),e.setSelectionRange(e.value.length,e.value.length),console.log("Filled font size from typography:",o)}catch(e){console.error("Failed to fill font size from typography",e)}},B.onclick=()=>{try{if(!Q||!Array.isArray(Q)||Q.length===0){alert("No typography styles defined. Please add typography styles or extract from Figma first.");return}let e=document.getElementById("line-height-scale");if(!e)return;let t=[];if(Q.forEach(a=>{let i=String(a.lineHeight||"").trim();if(i==="auto")t.push("auto");else{let c=parseFloat(i.replace("%",""));isNaN(c)||t.push(c)}}),!t.length){alert("No valid line heights found in typography styles.");return}let o=t.includes("auto"),l=t.filter(a=>a!=="auto"),n=Array.from(new Set(l)).sort((a,i)=>a-i),s=o?["auto",...n]:n;e.value=s.join(", "),ve(),e.focus(),e.setSelectionRange(e.value.length,e.value.length),console.log("Filled line height from typography:",s)}catch(e){console.error("Failed to fill line height from typography",e)}};let Gt=document.getElementById("typography-panel"),To=document.getElementById("typography-panel-header"),No=document.getElementById("typography-panel-toggle");To&&Gt&&No&&(To.onclick=()=>{let e=Gt.classList.contains("collapsed");Gt.classList.toggle("collapsed");let t=No.querySelector(".issue-group-toggle-icon");t&&(t.textContent="\u25B6")});let _t=document.getElementById("settings-panel"),Fo=document.getElementById("settings-panel-header"),$t=document.getElementById("settings-panel-toggle");if(Fo&&_t&&$t){let e=()=>{let t=_t.classList.contains("collapsed");_t.classList.toggle("collapsed");let o=$t.querySelector(".issue-group-toggle-icon");o&&(o.textContent="\u25B6")};Fo.onclick=t=>{t.target===$t||$t.contains(t.target)||e()},$t.onclick=t=>{t.stopPropagation(),e()}}function Jt(){let e=document.getElementById("color-preview-panel"),t=document.getElementById("color-preview-panel-header"),o=document.getElementById("color-preview-panel-toggle");if(!e||!t||!o){console.log("Color preview panel elements not found, retrying..."),setTimeout(Jt,100);return}console.log("Setting up color preview panel toggle");let l=()=>{let n=e.classList.contains("collapsed");e.classList.toggle("collapsed");let s=o.querySelector(".issue-group-toggle-icon");s&&(s.textContent="\u25B6"),console.log("Color preview panel toggled, isCollapsed:",!n)};t.onclick=n=>{n.preventDefault(),n.stopPropagation(),console.log("Color preview panel header clicked"),l()},o.onclick=n=>{n.preventDefault(),n.stopPropagation(),console.log("Color preview panel toggle button clicked"),l()}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Jt):Jt();function He(){let e=document.getElementById("color-scale"),t=document.getElementById("color-preview");if(!e||!t)return;let o=e.value.trim();if(t.innerHTML="",!o)return;let l=o.split(",").map(a=>a.trim()).filter(a=>a),n=1;l.forEach(a=>{let i=a.toUpperCase(),c=J[i];if(c&&/^No name \d+$/.test(c)){let u=parseInt(c.replace("No name ",""),10);u>=n&&(n=u+1)}});let s=1;l.forEach((a,i)=>{let c=/^#[0-9A-Fa-f]{3,8}$/.test(a),u=/^rgba?\(/.test(a);if(!c&&!u)return;let d=document.createElement("div");d.className="color-swatch-container";let b=document.createElement("div");b.className="color-swatch",b.style.background=a;let y=document.createElement("button");y.innerHTML="\xD7",y.className="color-swatch-close",y.onclick=function(A){var D;A.stopPropagation();let N=e.value.split(",").map(ye=>ye.trim()).filter(ye=>ye),j=(D=N[i])==null?void 0:D.toUpperCase(),z=N.filter((ye,x)=>x!==i);e.value=z.join(", "),j&&J[j]&&delete J[j],He(),typeof ve=="function"&&ve()};let f=a.toUpperCase(),m=J[f];if(!m){for(;Object.values(J).includes(`No name ${s}`);)s++;m=`No name ${s}`,J[f]=m,s++,typeof ve=="function"&&ve()}let g=document.createElement("div");g.className="color-swatch-label";let E=document.createElement("div");E.className="color-swatch-label-name",E.textContent=m,E.title="Click to edit name",E.style.cursor="pointer",E.onclick=function(A){A.stopPropagation();let N=document.createElement("input");N.type="text",N.value=m,N.className="color-swatch-name-input",N.style.cssText="width: 100%; font-size: 11px; padding: 2px 4px; border: 1px solid #3b82f6; border-radius: 4px; text-align: center; box-sizing: border-box;",E.style.display="none",E.parentNode.insertBefore(N,E),N.focus(),N.select();let j=()=>{let z=N.value.trim()||`No name ${s}`;J[f]=z,E.textContent=z,E.style.display="",N.remove(),typeof ve=="function"&&ve()};N.onblur=j,N.onkeydown=function(z){z.key==="Enter"?(z.preventDefault(),j()):z.key==="Escape"&&(E.style.display="",N.remove())}};let T=document.createElement("div");T.className="color-swatch-label-hex",T.textContent=f,g.appendChild(E),g.appendChild(T),b.appendChild(y),d.appendChild(b),d.appendChild(g),t.appendChild(d)})}function Yn(){var e,t,o,l,n,s;return{fontFamily:((e=document.getElementById("rule-font-family"))==null?void 0:e.checked)||!1,fontSize:((t=document.getElementById("rule-font-size"))==null?void 0:t.checked)||!1,fontWeight:((o=document.getElementById("rule-font-weight"))==null?void 0:o.checked)||!1,lineHeight:((l=document.getElementById("rule-line-height"))==null?void 0:l.checked)||!1,letterSpacing:((n=document.getElementById("rule-letter-spacing"))==null?void 0:n.checked)||!1,wordSpacing:((s=document.getElementById("rule-word-spacing"))==null?void 0:s.checked)||!1}}function Ge(){let e=document.getElementById("typography-table-body"),t=document.querySelector("#typography-table thead tr");if(!e||!t)return;let o=Yn(),l='<th class="typography-table-actions">Actions</th>';if(l+='<th class="typography-table-style-name" style="width: 120px;">Style Name</th>',o.fontFamily&&(l+='<th class="typography-table-font-family" style="width: 140px;">Font Family</th>'),o.fontSize&&(l+='<th class="typography-table-font-size" style="width: 80px;">Size (px)</th>'),o.fontWeight&&(l+='<th class="typography-table-font-weight" style="width: 100px;">Weight</th>'),o.lineHeight&&(l+='<th class="typography-table-line-height" style="width: 100px;">Line Height</th>'),o.letterSpacing&&(l+='<th class="typography-table-letter-spacing" style="width: 100px;">Letter Spacing</th>'),o.wordSpacing&&(l+='<th class="typography-table-word-spacing" style="width: 100px;">Word Spacing</th>'),t.innerHTML=l,Q.length===0){let n=Object.values(o).filter(s=>s).length+2;e.innerHTML=`
        <tr>
      ...
      `;return}e.innerHTML=Q.map(n=>{let s=`<tr data-id="${n.id}">`;return s+='<td class="typography-table-actions">',n.styleId&&(s+=`<button class="btn-table-action" onclick="selectTypographyStyle('${n.styleId}')" title="Select layer in Figma" style="background: #0071e3; color: white;">\u{1F441}</button>`),s+=`<button class="btn-table-action delete" onclick="deleteTypographyStyle(${n.id})" title="Delete">\u{1F5D1}</button>
        ...
        </td>`),o.lineHeight&&(s+=`<td><input type="text" value="${p(n.lineHeight)}" data-field="lineHeight" placeholder="120% or auto"></td>`),o.letterSpacing&&(s+=`<td><input type="text" value="${p(n.letterSpacing||"0")}" data-field="letterSpacing" placeholder="0 or 0.5px"></td>`),o.wordSpacing&&(s+=`<td><input type="text" value="${p(n.wordSpacing||"0")}" data-field="wordSpacing" placeholder="0"></td>`),s+="</tr>",s}).join(""),e.querySelectorAll("input, select").forEach(n=>{n.addEventListener("change",s=>{let a=s.target.closest("tr"),i=parseInt(a.dataset.id),c=s.target.dataset.field,u=s.target.value;Xn(i,c,u)})})}window.addTypographyStyle=function(){let e={id:nt++,name:"New Style",fontFamily:"Inter",fontSize:16,fontWeight:"Regular",lineHeight:"150%",letterSpacing:"0",wordSpacing:"0"};Q.push(e),Ge(),ve()};function Xn(e,t,o){let l=Q.find(n=>n.id===e);l&&(t==="fontSize"?l[t]=parseInt(o)||16:l[t]=o,ve())}window.deleteTypographyStyle=function(e){confirm("Delete this typography style?")&&(Q=Q.filter(t=>t.id!==e),X(),Ge(),ve())},window.selectTypographyStyle=function(e){parent.postMessage({pluginMessage:{type:"select-text-style",styleId:e}},"*")};let Bo=document.getElementById("btn-add-typo-style");Bo&&(Bo.onclick=()=>addTypographyStyle());let Ao=document.getElementById("btn-extract-typo-desktop");Ao&&(Ao.onclick=()=>{parent.postMessage({pluginMessage:{type:"extract-typography-styles",mode:"desktop"}},"*")});let Lo=document.getElementById("btn-extract-typo-tablet");Lo&&(Lo.onclick=()=>{parent.postMessage({pluginMessage:{type:"extract-typography-styles",mode:"tablet"}},"*")});let Ho=document.getElementById("btn-extract-typo-mobile");Ho&&(Ho.onclick=()=>{parent.postMessage({pluginMessage:{type:"extract-typography-styles",mode:"mobile"}},"*")});let qo=document.getElementById("btn-extract-typo-all");qo&&(qo.onclick=()=>{parent.postMessage({pluginMessage:{type:"extract-typography-styles",mode:"all"}},"*")});let Po=document.getElementById("btn-reset-typo-table");Po&&(Po.onclick=()=>{confirm(`\u26A0\uFE0F Reset typography table to default styles?

This will:
\u2022 Clear all current styles
\u2022 Restore default H1-H6 and Body styles

This action cannot be undone.`)&&(Q=[{id:1,name:"H1",fontFamily:"Inter",fontSize:48,fontWeight:"Bold",lineHeight:"120%",letterSpacing:"0",wordSpacing:"0"},{id:2,name:"H2",fontFamily:"Inter",fontSize:36,fontWeight:"Bold",lineHeight:"130%",letterSpacing:"0",wordSpacing:"0"},{id:3,name:"H3",fontFamily:"Inter",fontSize:30,fontWeight:"Semi Bold",lineHeight:"130%",letterSpacing:"0",wordSpacing:"0"},{id:4,name:"H4",fontFamily:"Inter",fontSize:24,fontWeight:"Semi Bold",lineHeight:"140%",letterSpacing:"0",wordSpacing:"0"},{id:5,name:"H5",fontFamily:"Inter",fontSize:20,fontWeight:"Semi Bold",lineHeight:"140%",letterSpacing:"0",wordSpacing:"0"},{id:6,name:"H6",fontFamily:"Inter",fontSize:16,fontWeight:"Semi Bold",lineHeight:"150%",letterSpacing:"0",wordSpacing:"0"},{id:7,name:"Body",fontFamily:"Inter",fontSize:14,fontWeight:"Regular",lineHeight:"150%",letterSpacing:"0",wordSpacing:"0"}],nt=8,Ge(),ve(),alert("\u2705 Typography table has been reset to default styles!"))}),Ge(),["rule-font-family","rule-font-size","rule-font-weight","rule-line-height","rule-letter-spacing","rule-word-spacing"].forEach(e=>{let t=document.getElementById(e);t&&t.addEventListener("change",()=>{Ge(),ve()})});let Ro=document.getElementById("color-scale");Ro&&(Ro.addEventListener("input",()=>{He()}),He());let Zn=mn({maxHistory:10,postPluginMessage:e=>parent.postMessage({pluginMessage:e},"*"),getCurrentReportData:()=>k,setIsViewingTokens:e=>{Ct=!!e},renderResults:Ve,renderTokens:It}),{saveScanHistory:Do,requestScanHistory:Wo,renderScanHistory:Yt,restoreReportFromHistory:hl,loadLastScanModeOnce:Kn,setHistory:Qn,clearLocalHistory:el}=Zn,tl=document.getElementById("export-group"),Bt=document.getElementById("export-dropdown");G.onclick=e=>{e.stopPropagation(),Bt.style.display=Bt.style.display==="block"?"none":"block"},document.querySelectorAll(".export-option").forEach(e=>{e.onclick=t=>{t.stopPropagation();let o=e.getAttribute("data-format");gn({format:o,reportData:k,getTypeDisplayName:Tt,colorNameMap:J}),Bt.style.display="none"}}),document.addEventListener("click",e=>{tl.contains(e.target)||(Bt.style.display="none")});let We,Be,$e,yt;function At(e){if(!e)return;let t=e.filter(i=>i.severity==="error"&&!i.ignored).length,o=e.filter(i=>i.severity==="warn"&&!i.ignored).length,l=e.filter(i=>!i.ignored).length,n=document.getElementById("filter-count-all"),s=document.getElementById("filter-count-error"),a=document.getElementById("filter-count-warn");n&&(n.textContent=l),s&&(s.textContent=t),a&&(a.textContent=o)}function Lt(){console.log("applyFilters called",{isViewingTokens:Ct,hasTokens:!!k.tokens,hasIssues:!!k.issues,currentFilter:Ae,currentSearch:Ie,currentColorTypeFilter:Oe});let e=document.querySelector(".report-tab.active"),t=e?e.getAttribute("data-tab"):null;t==="animations"?(console.log("Applying search to animations"),typeof window.searchAnimations=="function"&&window.searchAnimations(Ie)):t==="tokens"&&k.tokens?(console.log("Applying filters to tokens"),It(k.tokens,!1,{skipTabSwitch:!0})):t==="issues"&&k.issues?(console.log("Applying filters to issues"),Ve(k.issues,!1,{skipTabSwitch:!0})):Ct&&k.tokens?(console.log("Applying filters to tokens (fallback)"),It(k.tokens,!1,{skipTabSwitch:!0})):k.issues&&(console.log("Applying filters to issues (fallback)"),Ve(k.issues,!1,{skipTabSwitch:!0}))}function Xt(){if(console.log("setupFilterHandlers called"),We=document.getElementById("search-input"),Be=document.getElementById("btn-clear-search"),$e=document.querySelectorAll(".filter-btn"),yt=document.getElementById("color-type-select"),console.log("Elements found:",{searchInput:!!We,btnClearSearch:!!Be,filterButtons:$e?$e.length:0,colorTypeSelect:!!yt}),!We||!Be||!$e||$e.length===0){console.warn("Filter elements not found, retrying...",{searchInput:!!We,btnClearSearch:!!Be,filterButtons:$e?$e.length:0}),setTimeout(Xt,100);return}console.log("Setting up search input handler"),We.addEventListener("input",e=>{let t=e.target.value;console.log("Search input changed:",t),Ie=t,console.log("currentSearch set to:",Ie),Be&&(Be.style.display=Ie.trim()?"block":"none"),Lt()}),Be&&(console.log("Setting up clear search button handler"),Be.onclick=e=>{console.log("Clear search clicked"),e.preventDefault(),e.stopPropagation(),We&&(We.value=""),Ie="",Be.style.display="none",Lt()}),console.log("Setting up filter buttons handlers, count:",$e.length),$e.forEach((e,t)=>{console.log(`Setting up filter button ${t}:`,e.getAttribute("data-filter")),e.onclick=o=>{o.preventDefault(),o.stopPropagation();let l=e.getAttribute("data-filter"),n=e.classList.contains("active");console.log("Filter button clicked:",l,"isActive:",n),n&&l!=="all"?(e.classList.remove("active"),Ae="all",$e.forEach(s=>{s.getAttribute("data-filter")==="all"&&s.classList.add("active")})):($e.forEach(s=>s.classList.remove("active")),e.classList.add("active"),Ae=l),console.log("currentFilter set to:",Ae),Lt()}}),yt&&(console.log("Setting up color type select handler"),yt.addEventListener("change",e=>{console.log("Color type changed:",e.target.value),Oe=e.target.value,Lt()})),console.log("Filter handlers setup complete")}console.log("Setting up filter handlers, DOM readyState:",document.readyState),document.readyState==="loading"?(console.log("DOM still loading, waiting for DOMContentLoaded"),document.addEventListener("DOMContentLoaded",()=>{console.log("DOMContentLoaded fired, setting up handlers"),Xt(),lo(),so(),Wo()})):(console.log("DOM already ready, setting up handlers immediately"),Xt(),lo(),so(),Wo()),ae&&(C.onclick=()=>{console.log("Cancel operation clicked"),parent.postMessage({pluginMessage:{type:"cancel-scan"}},"*"),S.style.display="block",S.disabled=!1,I.style.display="block",I.disabled=!1,C.style.display="none",h.style.display="none"},oe.onclick=()=>{if(confirm(`\u26A0\uFE0F Are you sure you want to reset all settings to default and clear history?
...
This action cannot be undone.`)){document.getElementById("spacing-scale").value="0, 4, 8, 12, 16, 24, 32, 40, 48, 64, 72, 80, 88, 96",document.getElementById("spacing-threshold").value="100",document.getElementById("color-scale").value="",document.getElementById("font-size-scale").value="32, 24, 20, 18, 16, 14, 12",document.getElementById("font-size-threshold").value="100",document.getElementById("line-height-scale").value="auto, 100, 110, 120, 130, 140, 150, 160, 170",document.getElementById("line-height-threshold").value="300",document.getElementById("line-height-baseline-threshold").value="120",Q=[{id:1,name:"H1",fontFamily:"Inter",fontSize:48,fontWeight:"Bold",lineHeight:"120%",letterSpacing:"0",wordSpacing:"0"},{id:2,name:"H2",fontFamily:"Inter",fontSize:36,fontWeight:"Bold",lineHeight:"130%",letterSpacing:"0",wordSpacing:"0"},{id:3,name:"H3",fontFamily:"Inter",fontSize:30,fontWeight:"Semi Bold",lineHeight:"130%",letterSpacing:"0",wordSpacing:"0"},{id:4,name:"H4",fontFamily:"Inter",fontSize:24,fontWeight:"Semi Bold",lineHeight:"140%",letterSpacing:"0",wordSpacing:"0"},{id:5,name:"H5",fontFamily:"Inter",fontSize:20,fontWeight:"Semi Bold",lineHeight:"140%",letterSpacing:"0",wordSpacing:"0"},{id:6,name:"H6",fontFamily:"Inter",fontSize:16,fontWeight:"Semi Bold",lineHeight:"150%",letterSpacing:"0",wordSpacing:"0"},{id:7,name:"Body",fontFamily:"Inter",fontSize:14,fontWeight:"Regular",lineHeight:"150%",letterSpacing:"0",wordSpacing:"0"}],nt=8,Ge(),document.getElementById("rule-typo-style").checked=!0,document.getElementById("rule-font-family").checked=!0,document.getElementById("rule-font-size").checked=!0,document.getElementById("rule-font-weight").checked=!0,document.getElementById("rule-line-height").checked=!0,document.getElementById("rule-letter-spacing").checked=!1,document.getElementById("rule-word-spacing").checked=!1,Ge(),He(),k={issues:null,tokens:null,scanMode:null,timestamp:null,context:null,lastActiveTab:"issues"},zt("issues"),zt("tokens"),Le("issues"),el(),parent.postMessage({pluginMessage:{type:"clear-history"}},"*");let e=document.getElementById("history-panel");e&&e.style.display!=="none"&&Yt(),ve(),alert("\u2705 All settings have been reset to default and history has been cleared!")}},ae.onclick=()=>{let e=document.getElementById("history-panel");if(e){let t=e.style.display!=="none";e.style.display=t?"none":"flex",t||Yt()}}),Z&&(Z.onclick=()=>{let e=document.getElementById("history-panel");e&&(e.style.display="none")});let Uo=document.getElementById("btn-save-settings"),qe=document.getElementById("save-settings-modal"),we=document.getElementById("setting-name-input"),Ht=document.getElementById("btn-confirm-save-settings"),jo=document.getElementById("btn-cancel-save-settings"),Oo=document.getElementById("btn-close-save-settings");Uo&&(Uo.onclick=()=>{parent.postMessage({pluginMessage:{type:"get-project-name"}},"*"),qe&&(qe.style.display="flex",we&&(we.value="",we.focus(),we.onkeydown=e=>{e.key==="Enter"&&(e.preventDefault(),Ht&&Ht.click())}))}),Oo&&(Oo.onclick=()=>{qe&&(qe.style.display="none")}),jo&&(jo.onclick=()=>{qe&&(qe.style.display="none")});let Me=document.getElementById("replace-confirm-modal"),Vo=document.getElementById("replace-setting-name"),Go=document.getElementById("btn-confirm-replace"),_o=document.getElementById("btn-cancel-replace"),Jo=document.getElementById("btn-close-replace-confirm"),ze=null;function Yo(e,t){parent.postMessage({pluginMessage:{type:"save-settings",name:e,values:t,forceReplace:!0}},"*")}Ht&&(Ht.onclick=()=>{var o,l,n,s,a,i,c,u,d,b,y,f,m,g,E,T,A;let e=(o=we==null?void 0:we.value)==null?void 0:o.trim();if(!e){alert("\u26A0\uFE0F Please enter a setting name");return}let t={spacingScale:((l=document.getElementById("spacing-scale"))==null?void 0:l.value)||"",spacingThreshold:((n=document.getElementById("spacing-threshold"))==null?void 0:n.value)||"100",colorScale:((s=document.getElementById("color-scale"))==null?void 0:s.value)||"",colorNameMap:J,ignoredIssues:ue,fontSizeScale:((a=document.getElementById("font-size-scale"))==null?void 0:a.value)||"",fontSizeThreshold:((i=document.getElementById("font-size-threshold"))==null?void 0:i.value)||"100",lineHeightScale:((c=document.getElementById("line-height-scale"))==null?void 0:c.value)||"",lineHeightThreshold:((u=document.getElementById("line-height-threshold"))==null?void 0:u.value)||"300",lineHeightBaselineThreshold:((d=document.getElementById("line-height-baseline-threshold"))==null?void 0:d.value)||"120",typographyStyles:Q,skipNames:((b=document.getElementById("scan-skip-names"))==null?void 0:b.value)||"",typographyRules:{checkStyle:((y=document.getElementById("rule-typo-style"))==null?void 0:y.checked)||!0,checkFontFamily:((f=document.getElementById("rule-font-family"))==null?void 0:f.checked)||!0,checkFontSize:((m=document.getElementById("rule-font-size"))==null?void 0:m.checked)||!0,checkFontWeight:((g=document.getElementById("rule-font-weight"))==null?void 0:g.checked)||!0,checkLineHeight:((E=document.getElementById("rule-line-height"))==null?void 0:E.checked)||!0,checkLetterSpacing:((T=document.getElementById("rule-letter-spacing"))==null?void 0:T.checked)||!1,checkWordSpacing:((A=document.getElementById("rule-word-spacing"))==null?void 0:A.checked)||!1}};ze={name:e,values:t},parent.postMessage({pluginMessage:{type:"check-setting-name",name:e}},"*")}),Go&&(Go.onclick=()=>{ze&&(Yo(ze.name,ze.values),ze=null),Me&&(Me.style.display="none")}),_o&&(_o.onclick=()=>{ze=null,Me&&(Me.style.display="none"),we&&(we.focus(),we.select())}),Jo&&(Jo.onclick=()=>{ze=null,Me&&(Me.style.display="none"),we&&(we.focus(),we.select())}),Me&&(Me.onclick=e=>{e.target===Me&&(ze=null,Me.style.display="none",we&&(we.focus(),we.select()))});let Xo=document.getElementById("btn-load-settings"),Pe=document.getElementById("load-settings-modal"),Et=document.getElementById("settings-list"),Zt=document.getElementById("settings-empty-state"),Zo=document.getElementById("btn-close-load-settings");function Ko(e){if(!(!Et||!Zt)){if(!e||e.length===0){Et.innerHTML="",Zt.style.display="block";return}Zt.style.display="none",Et.innerHTML=e.map(t=>{let o=new Date(t.updatedAt||t.createdAt),l=o.toLocaleDateString()+" "+o.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"});return`
      ...
      `}).join(""),Et.querySelectorAll(".settings-item").forEach(t=>{let o=t.getAttribute("data-setting-name");t.onclick=l=>{l.target.closest(".btn-remove-setting")||(parent.postMessage({pluginMessage:{type:"load-settings",name:o}},"*"),Pe&&(Pe.style.display="none"))}}),Et.querySelectorAll(".btn-remove-setting").forEach(t=>{t.onclick=o=>{o.stopPropagation();let l=t.getAttribute("data-setting-name");confirm(`\u26A0\uFE0F Are you sure you want to remove "${l}"?`)&&parent.postMessage({pluginMessage:{type:"remove-settings",name:l}},"*")}})}}Xo&&(Xo.onclick=()=>{parent.postMessage({pluginMessage:{type:"get-saved-settings"}},"*"),Pe&&(Pe.style.display="flex")}),Zo&&(Zo.onclick=()=>{Pe&&(Pe.style.display="none")});let Qo=document.getElementById("btn-export-settings");Qo&&(Qo.onclick=()=>{var s,a,i,c,u,d,b,y,f,m,g,E,T,A,N;let e=((s=document.getElementById("color-scale"))==null?void 0:s.value)||"",t=e.split(",").map(j=>j.trim().toUpperCase()).filter(j=>j&&j.startsWith("#")),o=Ke({},J),l=1;Object.values(o).forEach(j=>{if(j&&/^No name \d+$/.test(j)){let z=parseInt(j.replace("No name ",""),10);z>=l&&(l=z+1)}}),t.forEach(j=>{o[j]||(o[j]=`No name ${l}`,l++)});let n={spacingScale:((a=document.getElementById("spacing-scale"))==null?void 0:a.value)||"",spacingThreshold:((i=document.getElementById("spacing-threshold"))==null?void 0:i.value)||"100",colorScale:e,colorNameMap:o,ignoredIssues:ue,fontSizeScale:((c=document.getElementById("font-size-scale"))==null?void 0:c.value)||"",fontSizeThreshold:((u=document.getElementById("font-size-threshold"))==null?void 0:u.value)||"100",lineHeightScale:((d=document.getElementById("line-height-scale"))==null?void 0:d.value)||"",lineHeightThreshold:((b=document.getElementById("line-height-threshold"))==null?void 0:b.value)||"300",lineHeightBaselineThreshold:((y=document.getElementById("line-height-baseline-threshold"))==null?void 0:y.value)||"120",typographyStyles:Q,typographyRules:{checkStyle:((f=document.getElementById("rule-typo-style"))==null?void 0:f.checked)||!0,checkFontFamily:((m=document.getElementById("rule-font-family"))==null?void 0:m.checked)||!0,checkFontSize:((g=document.getElementById("rule-font-size"))==null?void 0:g.checked)||!0,checkFontWeight:((E=document.getElementById("rule-font-weight"))==null?void 0:E.checked)||!0,checkLineHeight:((T=document.getElementById("rule-line-height"))==null?void 0:T.checked)||!0,checkLetterSpacing:((A=document.getElementById("rule-letter-spacing"))==null?void 0:A.checked)||!1,checkWordSpacing:((N=document.getElementById("rule-word-spacing"))==null?void 0:N.checked)||!1}};parent.postMessage({pluginMessage:{type:"get-project-name"}},"*"),window.pendingExportValues=n});let en=document.getElementById("btn-import-settings"),ht=document.getElementById("import-settings-modal"),_e=document.getElementById("import-settings-file-input"),tn=document.getElementById("btn-select-import-file"),Je=document.getElementById("import-file-info"),on=document.getElementById("import-file-name"),Re=document.getElementById("btn-confirm-import"),Kt=document.getElementById("btn-cancel-import"),Qt=document.getElementById("btn-close-import-settings"),bt=null,Ce=null;if(en&&(en.onclick=()=>{bt=null,Ce=null,_e&&(_e.value=""),Je&&(Je.style.display="none"),Re&&(Re.disabled=!0),ht&&(ht.style.display="flex")}),tn&&_e&&(tn.onclick=()=>{_e.click()}),_e&&(_e.onchange=e=>{let t=e.target.files[0];if(!t)return;if(!t.name.endsWith(".json")){alert("\u26A0\uFE0F Please select a JSON file");return}bt=t,on&&(on.textContent=t.name),Je&&(Je.style.display="block"),Re&&(Re.disabled=!1);let o=new FileReader;o.onload=l=>{try{Ce=JSON.parse(l.target.result),console.log("Imported settings data:",Ce)}catch(n){alert("\u274C Error parsing JSON file: "+n.message),bt=null,Ce=null,Je&&(Je.style.display="none"),Re&&(Re.disabled=!0)}},o.onerror=()=>{alert("\u274C Error reading file"),bt=null,Ce=null},o.readAsText(t)}),Re&&(Re.onclick=()=>{if(!Ce){alert("\u274C No file data to import");return}console.log("Import file data:",Ce);let e=null;if(Array.isArray(Ce)){if(Ce.length===0){alert("\u274C No settings found in file");return}if(e=Ce[0].values,!e){alert("\u274C Invalid settings format. No values found in setting object.");return}}else if(Ce.values)e=Ce.values;else if(Ce.spacingScale!==void 0||Ce.colorScale!==void 0)e=Ce;else{alert("\u274C Invalid settings file format. Expected an array of settings, a single setting object, or a values object."),console.error("Invalid import data structure:",Ce);return}if(!e||typeof e!="object"){alert("\u274C Invalid settings format. No values found.");return}console.log("Applying values:",e),Ut(e),ve(),ht&&(ht.style.display="none"),bt=null,Ce=null,_e&&(_e.value=""),Je&&(Je.style.display="none"),Re&&(Re.disabled=!0),alert("\u2705 Settings imported and applied to input fields successfully")}),Kt||Qt){let e=()=>{ht&&(ht.style.display="none"),bt=null,Ce=null,_e&&(_e.value=""),Je&&(Je.style.display="none"),Re&&(Re.disabled=!0)};Kt&&(Kt.onclick=e),Qt&&(Qt.onclick=e)}qe&&(qe.onclick=e=>{e.target===qe&&(qe.style.display="none")}),Pe&&(Pe.onclick=e=>{e.target===Pe&&(Pe.style.display="none")}),ee.onclick=()=>{parent.postMessage({pluginMessage:{type:"close"}},"*")},window.onmessage=e=>{var o,l;console.log("Received message:",e.data);let t=e.data.pluginMessage;if(t&&t.type==="fix-issue-result"){if(console.log("[fix-issue-result] Received:",{issueId:t.issueId,success:t.success,message:t.message}),re(t.issueId,t.message,t.success),!t.success){console.log("[fix-issue-result] Error detected, showing error modal...");let n=t.message||"An error occurred while fixing the issue.";console.log("[fix-issue-result] Error message:",n);try{to(n),console.log("[fix-issue-result] Error modal should be displayed")}catch(s){console.error("[fix-issue-result] Error showing error modal:",s),alert("Error: "+n)}}if(t.success){if(console.log("[fix-issue-result] Starting remove process for issueId:",t.issueId),console.log("[fix-issue-result] issueId type:",typeof t.issueId,"value:",t.issueId),k&&k.issues){let d=k.issues.length;k.issues=k.issues.filter(y=>String(y.id)!==String(t.issueId));let b=d-k.issues.length;console.log("[fix-issue-result] Removed",b,"issue(s) from data. Remaining issues:",k.issues.length)}let n=`.issue[data-issue-id="${t.issueId}"]`;console.log("[fix-issue-result] Trying selector1:",n);let s=document.querySelectorAll(n);console.log("[fix-issue-result] Found",s.length,"issue element(s) with this ID");let a=`button.btn-fix[data-id="${t.issueId}"]`,i=`button.btn-suggest-fix[data-id="${t.issueId}"]`;[...document.querySelectorAll(a),...document.querySelectorAll(i)].forEach(d=>{let b=d.closest(".issue");b&&!Array.from(s).includes(b)&&s.push(b)});let u=Array.from(new Set(Array.from(s)));if(console.log("[fix-issue-result] Total unique issue elements to remove:",u.length),u.length>0){let d=new Map;u.forEach((b,y)=>{console.log(`[fix-issue-result] Issue element ${y}:`,b),console.log(`[fix-issue-result] Issue element ${y} data-issue-id:`,b.getAttribute("data-issue-id")),console.log(`[fix-issue-result] Issue element ${y} data-issue-type:`,b.getAttribute("data-issue-type"));let f=b.closest(".issue-group");if(f){let m=f.getAttribute("data-issue-type");if(!d.has(m)){let E=f.querySelector(".badge");if(E){let T=parseInt(E.textContent)||0;d.set(m,{groupEl:f,badge:E,currentCount:T,removeCount:0})}}let g=d.get(m);g&&g.removeCount++}}),u.forEach((b,y)=>{b.style.transition="opacity 0.3s ease-out",b.style.opacity="0",setTimeout(()=>{b.parentNode&&(console.log(`[fix-issue-result] Removing element ${y} from DOM...`),b.remove())},300)}),setTimeout(()=>{let b=document.querySelectorAll(n);console.log("[fix-issue-result] After remove, remaining elements:",b.length),d.forEach((y,f)=>{let m=Math.max(0,y.currentCount-y.removeCount);y.badge.textContent=m,console.log(`[fix-issue-result] Updated badge for group "${f}" from ${y.currentCount} to ${m}`),m===0&&(y.groupEl.style.display="none",console.log(`[fix-issue-result] Hiding group "${f}" (no issues left)`))}),console.log("[fix-issue-result] Calling updateIssueCounts()..."),at(),console.log("[fix-issue-result] updateIssueCounts() completed"),console.log("[fix-issue-result] Re-rendering issues to sync UI..."),k&&k.issues&&Ve(k.issues,!1)},350)}else console.log("[fix-issue-result] No issue elements found! Trying to update counts anyway..."),at(),k&&k.issues&&Ve(k.issues,!1)}return}if(t&&t.type==="create-text-style-result"){re(t.issueId,t.message,t.success),t.success&&setTimeout(()=>{var s;let n=document.querySelector(`.issue[data-issue-id="${t.issueId}"]`)||((s=document.querySelector(`button.btn-create-style[data-id="${t.issueId}"]`))==null?void 0:s.closest(".issue"));n&&(n.style.transition="opacity 0.5s ease-out",n.style.opacity="0",setTimeout(()=>{if(n.parentNode){n.remove();let a=n.closest(".issue-group");if(a){let i=a.querySelector(".badge");if(i){let c=parseInt(i.textContent)||0,u=Math.max(0,c-1);i.textContent=u,u===0&&(a.style.display="none")}}}},500))},5e3);return}if(t&&t.type==="components-for-issue-loaded"){console.log("=== [components-for-issue-loaded] HANDLER CALLED ==="),console.log("[components-for-issue-loaded] Received message",t);let n=window.pendingComponentIssue;if(console.log("[components-for-issue-loaded] Pending issue:",n,"Message issueId:",t.issueId),console.log("[components-for-issue-loaded] Similar components:",t.similarComponents),n){let s=document.querySelector(`.issue[data-issue-id="${n.id}"]`);if(s){let a=s.querySelector("button.btn-suggest-fix");a&&(a.disabled=!1,a.style.opacity="1",a.style.cursor="pointer",a.dataset.originalText&&(a.textContent=a.dataset.originalText,delete a.dataset.originalText))}}if(t.similarComponents&&t.similarComponents.length>0){console.log("[components-for-issue-loaded] Showing suggest modal with",t.similarComponents.length,"similar components");let s=n||{id:t.issueId,nodeName:"Unnamed"};try{Bn(s,t.similarComponents),console.log("[components-for-issue-loaded] Modal function called successfully")}catch(a){console.error("[components-for-issue-loaded] Error showing modal:",a),alert("Error showing component suggestion modal: "+a.message)}window.pendingComponentIssue=null;return}else{console.warn("[components-for-issue-loaded] No similar components found"),alert("No similar components found. Please use 'Select Component' to choose from all components or 'Create New Component' to create a new one."),window.pendingComponentIssue=null;return}}if(t&&t.type==="all-components-loaded"){console.log("=== [all-components-loaded] HANDLER CALLED ==="),console.log("[all-components-loaded] Received message",t);let n=window.pendingSelectComponentIssue;if(console.log("[all-components-loaded] Pending issue:",n),console.log("[all-components-loaded] Message issueId:",t.issueId,typeof t.issueId),console.log("[all-components-loaded] Pending issueId:",n==null?void 0:n.id,typeof(n==null?void 0:n.id)),console.log("[all-components-loaded] Components:",t.components),console.log("[all-components-loaded] Components length:",t.components?t.components.length:0),n){let s=document.querySelector(`.issue[data-issue-id="${n.id}"]`);if(s){let a=s.querySelector("button.btn-select-component");a&&(a.disabled=!1,a.style.opacity="1",a.style.cursor="pointer",a.dataset.originalText&&(a.textContent=a.dataset.originalText,delete a.dataset.originalText))}}if(t.components&&t.components.length>0){console.log("[all-components-loaded] \u2713 Showing select modal with",t.components.length,"components");let s=n||{id:t.issueId,nodeName:"Unnamed"};console.log("[all-components-loaded] Issue to use:",s),console.log("[all-components-loaded] Calling showComponentSelectModal...");try{An(s,t.components),console.log("[all-components-loaded] \u2713 Modal function called successfully")}catch(a){console.error("[all-components-loaded] \u2717 Error showing modal:",a),console.error("[all-components-loaded] Error stack:",a.stack),alert("Error showing component selection modal: "+a.message)}window.pendingSelectComponentIssue=null;return}else{console.warn("[all-components-loaded] \u2717 No components available"),alert("No components found. Please use 'Create New Component' to create a new one."),window.pendingSelectComponentIssue=null;return}}if(t&&t.type==="figma-text-styles-loaded"){let n=window.pendingSuggestTextSizeIssue;if(n&&n.id===t.issueId){let u=n.fontSize||12,d=(t.styles||[]).filter(f=>f.fontSize>=14);if(d.length===0){let f=it(n);f?Ft(n,u,f,null,null):alert("No suitable text size match found (need >= 14px for ADA compliance). Please add font sizes to Font Size input or create text styles in Figma."),window.pendingSuggestTextSizeIssue=null;return}let b=null,y=1/0;d.forEach(f=>{let m=Math.abs(f.fontSize-u);m<y&&(y=m,b=f)}),b?Ft(n,u,b.fontSize,null,b):alert("No suitable text style found (need >= 14px for ADA compliance)"),window.pendingSuggestTextSizeIssue=null;return}let s=window.pendingTextSizeIssue;if(s&&s.id===t.issueId){Hn(s,t.styles||[]),window.pendingTextSizeIssue=null;return}let a=window.pendingTypographyStyleIssue;if(a&&a.id===t.issueId){if(t.error||!t.styles||t.styles.length===0){let u=document.querySelector(`.issue[data-issue-id="${t.issueId}"]`);if(u){let d=u.querySelector("button.btn-fix"),b=u.querySelector("button.btn-suggest-fix");d&&(d.style.display="none"),b&&(b.style.display="none")}window.pendingTypographyStyleIssue=null;return}bo(a,t.styles||[]),window.pendingTypographyStyleIssue=null;return}let i=window.pendingTypographyCheckIssue;if(i&&i.id===t.issueId){if(t.error||!t.styles||t.styles.length===0){let u=document.querySelector(`.issue[data-issue-id="${t.issueId}"]`);if(u){let d=u.querySelector("button.btn-style-dropdown"),b=u.querySelector("button.btn-suggest-fix");d&&(d.style.display="none"),b&&(b.style.display="none")}window.pendingTypographyCheckIssue=null;return}bo(i,t.styles||[]),window.pendingTypographyCheckIssue=null;return}let c=document.querySelector(`.style-dropdown-menu[data-issue-id="${t.issueId}"]`);if(c){let u=c.closest(".issue"),d=u?u.getAttribute("data-issue-id"):null,b=u?u.getAttribute("data-issue-type"):null;if(t.error){c.innerHTML=`<div style="padding: 8px 12px; color: #ef4444; font-size: 12px;">Error: ${p(t.error)}</div>`;let y=u?u.querySelector("button.btn-style-dropdown"):null;y&&(y.style.display="none");let f=u?u.querySelector("button.btn-suggest-fix"):null;f&&(b==="typography-style"||b==="typography-check")&&(f.style.display="none")}else if(t.styles&&t.styles.length>0){let y=null;if(d&&k&&k.issues){let f=k.issues.find(m=>m.id===d);f&&f.bestMatch&&(y=f.bestMatch.name)}c.innerHTML=t.styles.map(f=>`
            <div class="style-dropdown-item" data-issue-id="${t.issueId}" data-style-id="${f.id}" data-style-name="${p(f.name)}" style="padding: 8px 12px; cursor: pointer; font-size: 12px; ${y===f.name?"background: #e3f2fd; font-weight: 600;":""}" onmouseover="this.style.background='#f5f5f5'" onmouseout="this.style.background='${y===f.name?"#e3f2fd":"white"}'">
              ${p(f.name)} ${y===f.name?"\u2B50":""}
            </div>
          `).join(""),c.querySelectorAll(".style-dropdown-item").forEach(f=>{f.onclick=m=>{m.preventDefault(),m.stopPropagation();try{let T=f.getAttribute("data-issue-id");T&&parent.postMessage({pluginMessage:{type:"select-node",id:T}},"*")}catch(T){console.error("Failed to auto-select node for style dropdown item:",T)}let g=f.getAttribute("data-style-id"),E=f.getAttribute("data-style-name");if(u&&k&&k.issues){let T=k.issues.find(A=>A.id===d);if(T){let A=t.styles.find(N=>N.id===g);A&&(c.style.display="none",$o(T,A))}}}})}else{c.innerHTML='<div style="padding: 8px 12px; color: #999; font-size: 12px;">No text styles found in Figma</div>';let y=u?u.querySelector("button.btn-style-dropdown"):null;y&&(y.style.display="none");let f=u?u.querySelector("button.btn-suggest-fix"):null;f&&(b==="typography-style"||b==="typography-check")&&(f.style.display="none")}}return}if(t&&t.type==="contrast-colors-loaded"){let n=window.pendingContrastIssue;n&&n.id===t.issueId&&(qn(n,t.colors||[]),window.pendingContrastIssue=null);return}if(t&&t.type==="apply-typography-style-result"){if(re(t.issueId,t.message,t.success),t.success||to(t.message||"An error occurred while applying the style."),t.success){let n=["typography-check","typography-style","typography"];k&&k.issues&&(k.issues=k.issues.filter(s=>!(String(s.id)===String(t.issueId)&&n.includes(s.type)))),setTimeout(()=>{at(),k&&k.issues&&Ve(k.issues,!1)},350)}return}if(t&&t.type==="bind-color-variable-result"){re(t.issueId,t.message,t.success),t.success&&(k&&k.issues&&(k.issues=k.issues.filter(n=>!(String(n.id)===String(t.issueId)&&n.type==="color-variable"))),setTimeout(()=>{at(),k&&k.issues&&Ve(k.issues,!1)},350));return}if(t&&t.type==="scan-progress"){w&&$&&(w.style.width=t.progress+"%",$.textContent=t.progress+"% ("+t.current+"/"+t.total+")");return}if(t&&t.type==="node-count-result"){let n=t.count||0,s=t.mode||"page";n>je?confirm(`\u26A0\uFE0F Large Design Warning
...
Do you want to continue?`)&&Se?Mo(Se.scope):(S&&(S.disabled=!1,S.textContent="Scan Issues"),h&&(h.style.display="none")):Se&&Mo(Se.scope),Se=null;return}if(t&&t.type==="last-report"){t.report?hn(t.report):console.log("No last report stored");return}if(t&&t.type==="history-data"){Qn(t.history),Kn();let n=document.getElementById("history-panel");n&&n.style.display!=="none"&&Yt();return}if(t&&t.type==="input-values-data"){t.values?(Ut(t.values),console.log("Restored input values:",t.values)):console.log("No saved input values to restore");return}if(t&&t.type==="project-name"){let n=t.name||"settings";if(window.pendingExportValues){let s=window.pendingExportValues;window.pendingExportValues=null;let a={name:n,values:s,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()},i=JSON.stringify([a],null,2),c=new Blob([i],{type:"application/json"}),u=URL.createObjectURL(c),d=document.createElement("a");d.href=u;let b=n.replace(/[^a-z0-9]/gi,"_").toLowerCase();d.download=`${b}-settings.json`,document.body.appendChild(d),d.click(),document.body.removeChild(d),URL.revokeObjectURL(u),console.log("\u2705 Settings exported successfully");return}we&&(we.value=n);return}if(t&&t.type==="check-setting-name-result"){t.exists?(Vo&&ze&&(Vo.textContent=t.name||ze.name),Me&&(Me.style.display="flex")):ze&&(Yo(ze.name,ze.values),ze=null);return}if(t&&t.type==="save-settings-result"){t.success?(Me&&(Me.style.display="none"),qe&&(qe.style.display="none"),we&&(we.value=""),ze=null,Pe&&Pe.style.display!=="none"&&parent.postMessage({pluginMessage:{type:"get-saved-settings"}},"*")):(alert("\u274C Failed to save settings: "+(t.error||"Unknown error")),Me&&(Me.style.display="none"),ze=null);return}if(t&&t.type==="saved-settings-list"){Ko(t.settings||[]);return}if(t&&t.type==="project-name"){let n=t.name||"settings";if(window.pendingExportValues){let s=window.pendingExportValues;window.pendingExportValues=null;let a={name:n,values:s,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()},i=JSON.stringify([a],null,2),c=new Blob([i],{type:"application/json"}),u=URL.createObjectURL(c),d=document.createElement("a");d.href=u;let b=n.replace(/[^a-z0-9]/gi,"_").toLowerCase();d.download=`${b}-settings.json`,document.body.appendChild(d),d.click(),document.body.removeChild(d),URL.revokeObjectURL(u),console.log("\u2705 Settings exported successfully")}return}if(t&&t.type==="load-settings-result"){t.success&&t.values?(Ut(t.values),ve()):alert("\u274C Failed to load settings: "+(t.error||"Unknown error"));return}if(t&&t.type==="remove-settings-result"){t.success?Ko(t.settings||[]):alert("\u274C Failed to remove settings: "+(t.error||"Unknown error"));return}if(t&&t.type==="report"){S.style.display="block",S.disabled=!1,C.style.display="none",h.style.display="none",I.disabled=!1;let n=t.issues||[];k.context=t.context||null,Ve(n);let s=((o=document.querySelector('input[name="scope"]:checked'))==null?void 0:o.value)||"page";Do(s,"issues",{issues:n},t.context||null)}if(t&&t.type==="tokens-report")if(I.style.display="block",I.disabled=!1,C.style.display="none",h.style.display="none",S.disabled=!1,t.error)me.innerHTML=`<div class="error-message">Error: ${p(t.error)}</div>`,Le("tokens");else{k.context=t.context||null,It(t.tokens),R.disabled=!(t.tokens&&Array.isArray(t.tokens.spacing)&&t.tokens.spacing.length>0),_.disabled=!(t.tokens&&Array.isArray(t.tokens.colors)&&t.tokens.colors.length>0),L.disabled=!(t.tokens&&Array.isArray(t.tokens.fontSize)&&t.tokens.fontSize.length>0),U.disabled=!(t.tokens&&Array.isArray(t.tokens.lineHeight)&&t.tokens.lineHeight.length>0);let n=((l=document.querySelector('input[name="scope"]:checked'))==null?void 0:l.value)||"page";Do(n,"tokens",{tokens:t.tokens},t.context||null)}if(t&&t.type==="typography-styles-extracted")if(t.styles&&Array.isArray(t.styles)&&t.styles.length>0){let n=t.mode==="desktop"?"Desktop":t.mode==="tablet"?"Tablet":t.mode==="mobile"?"Mobile":"All";Q=t.styles.map((s,a)=>({id:a+1,name:s.name,styleId:s.styleId,fontFamily:s.fontFamily,fontSize:s.fontSize,fontWeight:s.fontWeight,lineHeight:s.lineHeight,letterSpacing:s.letterSpacing||"0",wordSpacing:s.wordSpacing||"0"})),nt=Q.length+1,X(),Ge(),ve(),alert(`\u2705 Successfully imported ${t.styles.length} ${n} typography styles from Figma!
