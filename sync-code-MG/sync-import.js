...
// ==== CONFIG (có thể chỉnh) ====
const ORIGIN_ROOT = "/Users/thien.ho/Projects/aemaacs-investments-azure/mandg-investments";
...
const PATTERNS = [
  "ui.frontend/src/html/data/global.js",
  "ui.frontend/src/main/webpack/js/base/index.ts",
  "ui.frontend/src/main/webpack/scss/base/6-components/components-imports.scss",
  "ui.frontend/src/main/webpack/scss/base/6-components/components-print-imports.scss",
  "ui.frontend/src/main/webpack/scss/infracapital/6-components/components-settings-imports.scss"
];

// ==== Helpers tên ====
const toKebabCase = s => s
  .replace(/([A-Z])/g, '-$1')     // Thêm dấu gạch ngang trước chữ hoa
  .replace(/[\s_]+/g, '-')        // Thay space và underscore bằng dấu gạch ngang
...
function isVariantPath(relPath) {
  return (
    relPath.includes("ui.frontend/src/html/data222/")
  );
}
...
// ==== Run ====
console.log(`\n🔄 Syncing import files...`);

// Sync từng file trong PATTERNS (không có {name} placeholder)
for (const pattern of PATTERNS) {
  const src = path.join(ORIGIN_ROOT, pattern);
  const dst = path.join(DEST_ROOT, pattern);
  const treatAsDir = isDirectoryPattern(pattern);
